...
- ✅ **QC7 Chat security M9-M12 ปิดครบ** (2026-07-12, commit `f3e64f9`) — M9 rate limit webchat (IP+conn 20/นาที + cap contact ใหม่ 60/ชม.) · M10 guest token = `crypto.randomUUID()` server-gen + httpOnly cookie `swc_<connId>` (เลิก localStorage/Math.random, ปิด IDOR) · M11 unit RBAC (list/getThread/sendReply/setStatus/assign/markRead กรอง `auth.active.unitAccess`, null-unit=ระดับระบบ) · M12 `getOrOpenConversation` = `$transaction`+`pg_advisory_xact_lock(hashtextextended(contactId))` + findOrCreateContact catch P2002. **test `scripts/qc-chat-security.mts`: pass-after 23/23 · fail-before 12/23.** tsc 0, ไม่ db push. ⚠️ **ยังไม่ deploy** — working tree มี account WIP ของอีก session; ต้อง deploy จาก tree ที่ไม่มี account WIP
