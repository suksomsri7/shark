...
- ✅ **QC7 Chat security M9-M12 ปิดครบ** (2026-07-12, commit `f3e64f9`) — M9 rate limit webchat (IP+conn 20/นาที + cap contact ใหม่ 60/ชม.) · M10 guest token = `crypto.randomUUID()` server-gen + httpOnly cookie `swc_<connId>` (เลิก localStorage/Math.random, ปิด IDOR) · M11 unit RBAC (list/getThread/sendReply/setStatus/assign/markRead กรอง `auth.active.unitAccess`, null-unit=ระดับระบบ) · M12 `getOrOpenConversation` = `$transaction`+`pg_advisory_xact_lock(hashtextextended(contactId))` + findOrCreateContact catch P2002. **test `scripts/qc-chat-security.mts`: pass-after 23/23 · fail-before 12/23.** tsc 0, ไม่ db push. ✅ **deploy Vercel prod แล้ว** (2026-07-12 via git worktree ที่ commit `4828b14` — สะอาด ไม่ ship account WIP; shark.in.th READY 200) · 🟡 **VPS staging รอ co-deploy พร้อม account** เมื่อ session บัญชีปิด R-A..R-E. ⚠️ **deploy ซ้ำใช้ worktree เท่านั้น — ห้าม `git stash`** (จะดึงไฟล์ account ออกจากมืออีก session)
- ✅ **Restaurant P1 LIVE** (2026-07-12): 27 files/15 model — เมนู/86 · โซน/โต๊ะ/QR · session · ออเดอร์+KDS · เช็คบิล→POS · storefront QR. verify dine-in loop ผ่าน (`scripts/qc-restaurant.mts`: menu→table→session dedup→order→KDS→checkout→close). partial unique index 1 OPEN session/table. defer: สต็อก/Recipe(P2), SSE, pickup UI, reports
