...
## 3. พนักงาน AI หลายตัว — "จ้าง ตั้งชื่อ มอบหมายงาน" (ยืนยันโดยเจ้าของ 2026-07-11)

**หลัก: พนักงาน AI 1 คน = ระบบ AI 1 instance (AppSystem type AI)** — ใช้กลไก "สร้างหลายชุด + เชื่อมระบบ" ที่มีอยู่แล้ว ตรงสถาปัตยกรรม "ทุกอย่างคือระบบ" เป๊ะ

**การ "จ้าง" พนักงาน AI 1 คน กำหนดได้:**
| มิติ | ตัวอย่าง | เก็บที่ |
|---|---|---|
| ชื่อ + avatar + บุคลิกภาษา | "น้องฟ้า" 👩 พูดเป็นกันเอง / "คุณบัญชี" 🧑‍💼 ทางการ | AppSystem.name + settings.persona |
| ตำแหน่ง (template) | ต้อนรับ / ขาย / บัญชี / การตลาด / ผู้ช่วยรวม / **custom** | settings.role → กำหนดชุด tools ตั้งต้น |
| คำสั่งเฉพาะร้าน (custom instructions) | "ห้ามรับจองวันจันทร์" "เรียกลูกค้าว่าคุณลูกค้าเสมอ" | settings.instructions (ต่อท้าย system prompt) |
| **ขอบเขตงาน** | เชื่อมกับระบบไหนบ้าง (จองคิว A + สมาชิก + KB) — เห็น/ทำได้เฉพาะนั้น | AppSystemUnit links + settings.systemIds |
| **สิทธิ์ + วงเงิน** (เหมือนพนักงานจริง!) | read-only / สร้าง DRAFT / **auto-approve ในวงเงิน** เช่น ลดราคาได้ ≤10%, ออกใบแจ้งหนี้เอง ≤10,000฿ — เกิน = ส่งขออนุมัติเจ้าของ | settings.permissions {action: limit} |
| **เวลาทำงาน** | 24 ชม. / เฉพาะนอกเวลาที่คนตอบ (18:00-09:00 AI รับช่วง LINE) | settings.schedule |
| ช่องทาง | แชทใน dashboard / LINE / webchat หน้าร้าน | settings.channels |
| งบเครดิต/เดือน | จำกัด token ต่อพนักงาน (คุมค่าใช้จ่ายรายตัว) | settings.creditLimit + AiUsage ต่อ systemId |

**Templates ตั้งต้น (ตำแหน่ง):**
| ตำแหน่ง | tools ตั้งต้น | ตัวอย่างสั่งงาน |
|---|---|---|
| 🤵 ผู้ช่วยรวม (แนะนำเริ่มตัวแรก) | ทุกระบบที่ user มีสิทธิ์ | "วันนี้ยอดเท่าไหร่" "มีนัดกี่คิว" |
| 💁 ต้อนรับ | Booking, Queue, Member, KB | "ลงนัดคุณสมชายพรุ่งนี้บ่ายสอง" + ตอบลูกค้า LINE (P3) |
| 💰 ขาย | POS, Member, Coupon, Reward | "เปิดบิลตัดผม+สระ เบอร์ 081..." "แจกคูปอง Gold" |
| 🧾 บัญชี | Account, POS | "ออกใบแจ้งหนี้บริษัท X 5,350" "เดือนนี้กำไรเท่าไหร่" |
| 📣 การตลาด | Member, Coupon, Chat | วิเคราะห์ลูกค้า+ร่างแคมเปญ (§8b.5) |

**วงจรชีวิตแบบ HR (ภาษาที่ร้านเข้าใจ):** จ้าง (สร้าง) → มอบหมายงาน (เชื่อมระบบ) → **ประเมินผลงาน** (หน้า "ผลงาน" ต่อคน: นัดที่จอง/แชทที่ตอบ/เอกสารที่ร่าง/เวลาที่ประหยัดโดยประมาณ/คะแนนโหวตจากลูกค้า) → พักงาน (pause) → เลิกจ้าง (archive)

**กติกาความปลอดภัย (เพิ่มจาก §5):**
- สิทธิ์พนักงาน AI ⊆ สิทธิ์ของขอบเขตที่ตั้ง — **ไม่มีวันเกินสิทธิ์เจ้าของที่จ้าง** และ auto-approve ในวงเงินต้องมี audit log ทุกรายการ + สรุปใน Daily Brief ("เมื่อวานน้องฟ้าอนุมัติเอง 3 รายการ")
- พนักงาน AI **ไม่คุยกันเอง** (v1) — ส่งต่องานข้ามตัวผ่าน DRAFT/Kanban card ที่คนเห็น (กัน loop/พฤติกรรมคาดไม่ถึง)
- Memory + การเรียนรู้แยกต่อตัว (น้องฟ้าจำแบบของน้องฟ้า — ลง KB หมวด internal ของตัวเอง)
- UX guard: ร้านใหม่แนะนำจ้าง **1 ตัว (ผู้ช่วยรวม)** ก่อน — อย่าให้กดสร้าง 5 ตัวแล้วงง

**Pricing ที่เปลี่ยนไป (ดีขึ้น):** ขายเป็น "จ้างพนักงาน AI" รายตัว/เดือน (เช่น ตัวแรกฟรี jobs จำกัด, ตัวถัดไป 299-599฿/เดือน ตามตำแหน่ง) — เทียบค่าแรงพนักงานจริง = story การขายที่แรงกว่าขาย token/credit มาก
