...
> คู่มือแบบ: `ledger/DESIGN-CRM.md` (§9 มติ · C7 portal ยืนยันแล้ว 11 ก.ย.) · ภาพ 17 ใบ: `ledger/design-crm/` · API: `docs/api/CRM-API.md` · แผนงาน: `ledger/CRM-RUN.md`
...
| **C7** | Portal B2B | **ทำ** (เจ้าของยืนยัน 11 ก.ย. หลังอ่านคำอธิบายใน DESIGN-CRM §9) · เฟส C3 ใบ C3.5 · นิยาม: หน้าเว็บที่ *ลูกค้าบริษัท* เข้าเอง (ลิงก์อีเมล+OTP หรือ LINE) เพื่อดู/ตอบรับใบเสนอราคา · ดู/ชำระใบแจ้งหนี้ · โหลดใบเสร็จ/สัญญา · แจ้งเรื่อง — แทนการส่ง PDF ไป-มาทางอีเมล/LINE · ไม่เห็นดีล/สถานะการขายภายใน | §3.8 §5.9 §6.5 |
...
## 15. ภาคผนวก 18 ก.ย. 2569 — มติเพิ่ม C15–C27 หลังทบทวนแบบกับโค้ดจริง (main `6fff98d`)

> ที่มา/หลักฐานทุกข้อ: `ledger/REVIEW-CRM-DESIGN-2026-09-18.md` · แผนงาน: `ledger/CRM-MASTER-PLAN.md` · **ชื่อฟังก์ชัน/ตารางในหัวข้อ §4–§9 ที่ไม่ตรงโค้ดจริง ให้ยึดตาราง REVIEW §3** (พิมพ์เขียวเขียนก่อนงานสมาชิก M3.3–M3.11 และรอบแก้ตาม audit)

| มติ | เรื่อง | ทับหัวข้อ |
|---|---|---|
| **C15** | Portal B2B อยู่ที่ `/b/{slug}/*` (เส้นทาง `/p/[slug]` เป็นของโมดูล PAGES แล้ว) · login ใช้ `customer-session` ตัวเดิมที่ขยายให้ชี้ผู้ถือสิทธิ์แบบผู้ติดต่อ CRM ผ่าน `CrmPortalAccess` · คำว่า `platform_auth` ในเอกสารนี้ = ชุด session ลูกค้าดังกล่าว ไม่ใช่ตาราง session พนักงานหลังบ้าน | §2.2 · §3.13 · §5.9 · §6.4 · §11.8 |
| **C16** | ไม่เพิ่ม cron entry ใน `vercel.json` · งานถี่ (sequence · อีเมลตั้งเวลา · เตือนนัด) เข้าตัวกระจายงานรายนาทีที่ขี่ `/api/cron/outbox` · งานรายชั่วโมง/รายวันขี่ `/api/cron/hourly` และ `/api/cron/tick` · ทุกงานจองแถวแบบ lease | §7.5 |
| **C17** | ไฟล์ส่วนตัว: ไฟล์เสียงสนทนา · ไฟล์แนบอีเมล · เอกสาร/สลิป portal เสิร์ฟผ่าน route ตรวจสิทธิ์ + ลิงก์หมดอายุ 15 นาที (storage วันนี้เป็น URL CDN ถาวร) | §9 แถวของกลาง · §12 |
| **C18** | ตัวรันแอ็กชันสื่อสาร + WAIT ของ journey สมาชิก (M3.3 เสร็จแล้ว) **ย้าย** เป็นของกลางให้ CRM ใช้ร่วม — ไม่สร้างชุดที่สอง | §7.3 · C13 |
| **C19** | ไฟล์แนบ (`CrmFileLink`) + โน้ตปักหมุด + คอมเมนต์ภายใน @mention บนผู้ติดต่อ/บริษัท/ดีล | §3.3–3.5 · §4.3 · §5.5 |
| **C20** | ความยินยอมรายช่องทางของผู้ติดต่อที่ไม่ใช่สมาชิก (`CrmContactConsent`) · เป็นสมาชิกแล้วอ่านจาก `MemberConsent` ที่เดียว · ตรวจ **ตอนส่งจริง** ทุกเส้นทาง | §4.3 · §5.6 · §11.4 |
| **C21** | อายุการเก็บ lead ที่ไม่แปลง: `settings.crm.retention.leadMonths` (ค่าเริ่มต้น 24 · ปิดได้) | §11.7 |
| **C22** | ตั้งค่าแจ้งเตือนรายคน (ทับค่าร้าน) + quiet hours ฝั่งพนักงาน · ช่องทางรอบแรก push/อีเมล/ในแอป (LINE ถึงพนักงานรอเจ้าของตัดสิน) | §7.4 |
| **C23** | ตัวสลับ `settings.crm.uiVersion = 1\|2` ต่อระบบ (ค่าเริ่มต้น 1) + สวิตช์ `bridgesEnabled` — ทางถอยบน prod | §0.2 · §13 |
| **C24** | ฟอร์ม lead: honeypot + จำกัดอัตราใน DB + เวลากรอกขั้นต่ำ + ตัวเลือก Turnstile | §3.11 · §5.7 |
| **C25** | วันทำการ/วันหยุด: `settings.crm.businessDays[]` + `settings.crm.holidays[]` | §11.5 |
| **C26** | ข้อมูลทดสอบขนาดจริงสำหรับเกณฑ์ §12 (20,000 ดีล · 200,000 ผู้ติดต่อ) | §12 |
| **C28** | คอลัมน์เงินใหม่เป็น BigInt (`CrmDeal.paidSatang` · `wonValueSatang` · `CrmCommission.*`) · `valueSatang` คง Int + เพดาน ฿20 ล้าน/ดีล | §4.1 · §4.3 |
| **C29** | ไม่มี `PosSale.dealId` — ใช้ตาราง `CrmDealPayment (dealId, refType, refId, satang, status)` เป็นทั้งตัวผูกบิล↔ดีลและธงกันนับซ้ำ · ไม่มี `MktRecipient.emailMessageId` (แคมเปญสมาชิกใช้ pixel ของตัวเองต่อ) | §4.1 · §7.2 · §9 แถว POS/MARKETING |
| **C30** | ตัวกรองของคีย์ API (`ownerUserId`/`teamId`) เก็บเป็น pseudo-scope ใน `scopesJson` (`crm.filter.owner:<id>` · `crm.filter.team:<id>`) — ไม่มีคอลัมน์ `ApiKey.bundle` · **C16 ฉบับแก้**: เส้น `/api/cron/outbox` ไม่มีตัวยิงรายนาทีที่ยืนยันได้ → งานถี่รันด้วยตัวรัน cron บน VPS แบบเดียวกับ cron บัญชี (ติดตั้งจริงตอนปล่อย หลังเจ้าของอนุญาต) · **C15 ฉบับแก้**: session ของ portal ใช้ตารางพี่น้อง `PortalSession` (เพราะ `CustomerSession.customerId` เป็น NOT NULL) โดยใช้ตรรกะ OTP/token/คุกกี้/ตัวจำกัดอัตราร่วมกัน · ข้อตัดสินละเอียดทั้งหมด: `ledger/crm-briefs/crm-brief-RESOLUTIONS.md` | §4.1 · §6.2 · §7.5 · §5.9 |
| **C27** | กลุ่มข้อสอบ X1–X10 บังคับทุกใบงาน (ขอบเขต · คีย์/AI · ยิงพร้อมกัน · ส่งซ้ำ · จองแถว · ข้อมูลเข้าอันตราย · ผิวสาธารณะ · PDPA · การกระทำอันตราย · ความลับ/ไฟล์) | §13 |

แก้ความไม่สอดคล้องในเอกสารนี้: `TeamMember` เพิ่ม `acceptingLeads Boolean @default(true)` (§4.3) · เพิ่มตาราง `EmailDomain` (tenantId · domain · status · dkim/spf records · verifiedAt) (§4.3) · migration ยึดชุด `crm_v2_a` (เฟส C1 รวม `CrmVisibilityPolicy` `CrmFileLink` `CrmContactConsent`) · `crm_v2_b` (เฟส C2 ทั้งเฟส) · `crm_v2_c` (เฟส C3 ทั้งเฟส) (§4.6) · event ของระบบธุรกิจ 6 ตัว (`ticket.order.paid` `rental.returned` `school.enrolled` `hotel.checked_out` `clinic.visit.done` `queue.served`) และ `hr.payroll.paid` **ยังไม่มีจริง** ต้อง emit ใหม่ในโมดูลเจ้าของ (§7.2 · §9) · `inventory.item.updated` ไม่มี — ป้าย "ราคาเปลี่ยน" คำนวณตอนเปิดดีลแทน (§7.2)
