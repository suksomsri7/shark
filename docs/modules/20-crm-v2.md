...
แก้ความไม่สอดคล้องในเอกสารนี้: `TeamMember` เพิ่ม `acceptingLeads Boolean @default(true)` (§4.3) · เพิ่มตาราง `EmailDomain` (tenantId · domain · status · dkim/spf records · verifiedAt) (§4.3) · migration ยึดชุด `crm_v2_a` (เฟส C1 รวม `CrmVisibilityPolicy` `CrmFileLink` `CrmContactConsent`) · `crm_v2_b` (เฟส C2 ทั้งเฟส รวม `PosSale.dealId`) · `crm_v2_c` (เฟส C3 ทั้งเฟส) (§4.6) · event ของระบบธุรกิจ 6 ตัว (`ticket.order.paid` `rental.returned` `school.enrolled` `hotel.checked_out` `clinic.visit.done` `queue.served`) และ `hr.payroll.paid` **ยังไม่มีจริง** ต้อง emit ใหม่ในโมดูลเจ้าของ (§7.2 · §9) · `inventory.item.updated` ไม่มี — ป้าย "ราคาเปลี่ยน" คำนวณตอนเปิดดีลแทน (§7.2)
