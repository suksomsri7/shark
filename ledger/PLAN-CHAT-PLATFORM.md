...
| **`chat.message.sent`** 🆕 | แอดมินตอบ (ไม่รวม internal note) | `{ conversationId, messageId, externalUserId, channel, preview, body, senderName }`<br>🔴 `preview` ถูกตัด 140 ตัว (ใช้แจ้งเตือน) · **`body` = ข้อความเต็ม** (ใช้แสดงเป็นข้อความจริง) |
...
- 29 ส.ค. 2026 — 🔴 **โหมด dual ใช้งานจริงไม่ได้ในชั่วโมงแรก** เจ้าของทดสอบเองแล้วเจอ:
  ส่งจากเว็บ → เห็นใน SHARK ✅ → **ทีมตอบใน SHARK → ฝั่งเว็บลูกค้าว่างเปล่า** ❌
  เหตุ: ตัวรับ webhook ของ siamdive2 **ส่งแค่ push ไม่ได้เขียนคำตอบกลับเข้า `SupportMessage`**
  แต่โหมด `dual` **อ่านจาก DB เดิม** ⇒ คำตอบที่อยู่แต่ใน SHARK ไม่มีทางโผล่บนหน้าเว็บ
  **แก้**: เพิ่ม `mirrorReply` ใน deps → เขียนกลับ `SupportMessage` + อัปเดต `lastMessageAt/lastSender`
  🔴 **ลำดับสำคัญกว่าตัวโค้ด**: ต้องเขียนกลับ **ก่อน** ด่านอีเมล — ของเดิม `if (!email) return`
  ออกจากฟังก์ชันก่อน ⇒ ถ้าวางไว้หลังด่านนั้น **guest ทั้งหมดจะไม่เห็นคำตอบเลย** ซึ่งคือผู้ใช้
  ส่วนใหญ่ของแชทนี้ (ทักได้โดยไม่ต้องล็อกอิน) · ข้อสอบ **D3.2** ล็อกไว้แล้ว (พิสูจน์ fail-before)
  🔴 และ payload เดิมมีแต่ `preview` ที่ถูกตัด 140 ตัว ⇒ ข้อความยาวจะโดนตัดกลางคันเมื่อเอาไปแสดงจริง
