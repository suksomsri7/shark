...
## 7. ผลรอบแก้ (FIX-RUN S1–S4 · 17 ก.ย. 2569) — เจ้าของสั่ง "ทำให้ครบ"

วิธีทำ: ผู้เขียนข้อสอบ (Opus ×4 · อิสระจาก builder) เขียน `scripts/qc-member-fix-s{1..4}.mts` ให้ **แดงบนโค้ดเดิมก่อน** → builder (Opus ×4 · แบ่งไฟล์เจ้าของไม่ชนกัน · ใบงาน `ledger/member-briefs/member-fix-*.md`) แก้ → Fable seed ใหม่แล้วรันซ้ำเอง + build + ภาพ + qc:all · ไม่มี migration/แก้ schema ทั้งรอบ

| ชุด | ข้อสอบใหม่ (ก่อน → หลัง) | ปิดข้อ |
|---|---|---|
| S1 ความปลอดภัย/สิทธิ์ | 5/28 → **28/28** | H1 (กว้างกว่าที่ audit เขียน: `canReadMember` คืน true ให้คีย์ทุกโมดูล — แก้ที่ราก) · H2 · H3 · H4 · M3 · M4 · M6 (ครึ่ง suspend) · M15 · L1 · L4 · L5 · L7 · L9 · L10 |
| S2 เงิน/แต้ม/คิว | 5/25 → **25/25** (×3 รอบติด — มีข้อแข่งกันจริง) | H5 · M9 · M10 · M11 · M12 · L12 · L13 · H6 (ฝั่ง consumer ส่ง eventId) · L15 (กันเพดาน+WARN) |
| S3 ข้อความ/journey | 1/14 → **14/14** | H6 · H7 · M7 · M8 · L11 · L14 |
| S4 PDPA/รายงาน/เว็บฮุก | 7/30 → **30/30** | M1 · M2 · M5 · M6 (ครึ่ง erase) · M13 · M14 · L2 · L3 · L6 · L8 + **ของแถม**: เว็บฮุกของกฎอัตโนมัติบอร์ดงาน (`kanban/automation.ts`) ไม่ผ่านตัวกัน SSRF — ปิดแล้ว |

ตัวเลขพิสูจน์บั๊กก่อนแก้ (จากข้อสอบ S2): บิล 10 ใบของสมาชิกคนเดียวยิงพร้อมกัน ยอดสะสมได้ 20,500/104,500 · แลกแต้มพร้อมกัน 10 ครั้งจากยอด 100 ผ่านหมด ยอด −900 · VOID ที่ไม่เคยถูกบวก ทำยอด −55,000

**ข้อตัดสินของผู้คุมงาน (Fable)**
- `compose`: builder ทำให้ "ของแถมล้ม = event ล้ม" — **ตีกลับเฉพาะจุด**: ของแถมล้ม = WARN ไม่โยนต่อ (คงสัญญาเดิม) · งานหลักล้ม = ของแถมยังวิ่ง แล้วโยนท้ายสุดให้ retry
- เว็บฮุกของ event ที่ถูก retry: ยิงเฉพาะรอบแรก (`OutboxEvent.attempts === 0`) — ตาราง delivery ไม่มีคอลัมน์อ้าง event
- export ที่มีฟิลด์อ่อนไหว: บันทึกการเข้าดูรายคนเหมือนเดิม (D17) แต่เขียนเป็นก้อน `logAccessMany` (createMany ทีละ 1,000 + outbox ใบเดียว)
- สมาชิกที่ถูกระงับ: เพิกถอน session + เข้าสู่ระบบไม่ได้ (ไม่พบข้อความในพิมพ์เขียวที่อนุญาต) · ปลดระงับแล้วเข้าได้ทันที
- push token: ไม่ย้ายเครื่องข้ามบัญชี · ปลดผูกด้วย `DELETE /me/push-devices/{id|expoToken}` ตอนออกจากระบบ
- host ที่ resolve ไม่ได้ = ไม่บล็อก (ส่งไม่ถึงอยู่แล้ว) · ช่องทดสอบ `WEBHOOK_ALLOW_PRIVATE=1` ใช้ได้เฉพาะ `APP_ENV ≠ production`

**ORACLE-EDIT รอบนี้**: `WB-3.2` และ `M3.10-S3.10b` — เลื่อน `updatedAt` ของใบที่ล้มให้ถึงรอบ backoff ก่อนวัด (เงื่อนไขเดิมไม่เปลี่ยน) · seed บัญชี V2 ตั้ง `WEBHOOK_ALLOW_PRIVATE` ก่อนสร้างปลายทาง 127.0.0.1

**เลื่อน (ต้องแก้ schema/มี migration — รอเจ้าของสั่งแยก)**
1. `Customer.totalSpentSatang` Int → BigInt (ตอนนี้ตรึงเพดาน ฿21.47 ล้าน + WARN)
2. เก็บ PIN ใบสแตมป์เป็น hash (ตอนนี้ไม่ออกจากเซิร์ฟเวอร์ + จำกัดการเดา + เทียบ constant-time แล้ว แต่ในฐานยังเป็นข้อความ)
3. unique index `(systemId, customerId, event, channel, refId)` บน `MemberNotification` (ตอนนี้กันซ้ำด้วย advisory lock)
4. คอลัมน์เจ้าของบทสนทนา AI (`AiConversation`) สำหรับ L10 · คอลัมน์อ้าง event บน `WebhookDelivery`
5. PII ตกค้างหลัง erase ที่ยังไม่แตะ: `AutomationRun.payload` · `AiMessage` · `AuditLog.after` รุ่นเก่า
6. H3 แบบชัดแจ้ง: ส่งสิทธิ์ของผู้ใช้ผ่าน `ai/tools.ts#ToolCtx` แทนการอ่านจาก session (ตอนนี้ถูกต้องแล้วแต่เป็น implicit)
7. บั๊กแฝงนอกระบบสมาชิก: `account/gl.ts#nextJournalNo` = count+1 พังถาวรถ้ามีการลบรายการ (ควรเป็น max+1) — ส่งต่อ session บัญชี
8. ฐาน QC ใช้ร่วม: `drainOutbox` ไม่แยกร้าน + การเชื่อมต่อหมดเวลาเมื่อรันขนาน = ต้นเหตุ flake ของ qc:all (m2.8/m3.6 รอบ audit)
