...
2. Private files (C0.4): same Bunny storage zone, unguessable path, ~~`FileAsset.kind` prefixed `private:`~~ — 🔴 **แก้ 17 ก.ย. 2569 (ผู้คุมงาน · ตอนทำ C0.4)**: `FileAsset.kind` เป็น **enum** (`LOGO`/`ATTACHMENT`) จึงเติมคำนำหน้าไม่ได้ถ้าไม่แก้สคีมา และใบ C0.4 ห้ามมี migration ⇒ **ตัวบอกความเป็นส่วนตัวคือ "เส้นทางไฟล์"** `t/<tenantId>/private/<สุ่ม 160 บิต>.<ext>` และ `cdnUrl` เก็บ sentinel `private://<path>` ที่ไม่ใช่ URL ใช้งานได้ · ข้อสอบ `C0.4-S2.10` อ่าน `information_schema` + `pg_enum` ยืนยันเชิงกลไกว่าไม่มีการแก้สคีมาและมติเดิมทำไม่ได้จริง · ที่เหลือของข้อนี้ยังใช้: the CDN URL is NEVER returned or stored in a DTO; bytes are streamed by the authorised route using the storage API key. No new column.
