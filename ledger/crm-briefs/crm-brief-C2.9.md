...
## Controller ruling — หลัง builder (25 ก.ย. · Fable · binding)
builder: 45/47 → (ORACLE-EDIT X4 fixture โดยผู้คุมงาน) → **47/47 ไฟล์จริง** · ข้อ 5 แก้แล้ว: rental `returnAsset` / school `markPaid` / clinic `billVisit` หา POS **ก่อน** claim+emit (โยนก่อนแตะสถานะ · clinic fee 0 ไม่ต้องมี POS) — before: state revert แต่ event 1 + VISIT 1 · after: 0/0 · ชุดโมดูล 10 ชุดเขียว · ticket/hotel ไม่มีทาง revert-after-emit (ข้ามเมื่อไม่มี POS) · มติที่ยืนยัน: `booking.no_show` ไม่มี CRM extra (backlog enum) · `refType/refId` ลง audit เท่านั้น · `markCustomerFromBridge` emit `crm.contact.updated` เมื่อเปลี่ยนจริง · ticket markPaid claim แบบ conditional · หนี้บันทึก: crash ระหว่าง claim+emit กับ `pos.createSale` = ธุรกรรมจบแต่ไม่มีบิล POS (ของเดิม · เท่ากับหนี้ `shop.order.paid`)

## Controller ruling round 2 (25 ก.ย. · Fable · binding — หลังผู้ตรวจของผู้คุมงาน)
ผู้ตรวจยืนยัน (a)–(g) ทั้งหมด · ไม่มี BLOCKER · สั่งแก้: F1 คีย์ `crm.contact.updated#…#lifecycle-customer` คงที่ ⇒ win-back (CUSTOMER→CHURNED→ซื้อใหม่) ไม่ยิงกฎ/webhook → `newSeq()` · F2 audit ต้องมี `before.lifecycleStage` · F3 กฎ lifecycle ห้ามซ้ำ → derive จาก `canAdvanceLifecycle` · F4 Party มาช้า (partyId null ตอนสะพานวิ่ง) = กิจกรรมหายตลอดกาล → WARN ids-only + **หนี้ C6.1**: backfill party-links ต้อง re-emit/รันสะพานซ้ำ · N5/N8/N9 แก้ · **DEFERRED ที่ต้องประกาศ**: "score hook" (C2.8 `SCORE_RULE_EVENTS` = AUTOMATION_EVENTS ครอบ event ธุรกิจอยู่แล้ว — ยืนยันตอนรับ C2.8) · "RENEWAL deal via rules" ทำไม่ได้เพราะ `CRM_RULE_TRIGGERS` กรองเฉพาะ `crm.*`/`custom.record.*` → เปิด trigger จาก event ธุรกิจใน **C3.6 (integrations)** · N6 ticket race-loser ไม่ post บิล (รับ · เท่ากับหนี้เดิม) · N13 shop ยัง claim-then-revert (ปลอดภัยเพราะ emit หลัง createSale · บันทึก handover) · expected.json ไม่เข้า commit · `qc-member-m2.8` แดง = oracle rot (log อายุ/ภาพ) — จะหายตอนปิดเฟส C2 ด้วย `qc:all`
- ORACLE-EDIT (ผู้เขียนข้อสอบ ใน c23): `S10.1–S10.5` (win-back · 2 ระบบ = VISIT ละระบบ · no-POS pre-gate · late Party WARN · static ไม่ซ้ำ engine)
