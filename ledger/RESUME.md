...
### 🔴 ยังยื่นไม่ได้ — 2 อย่างที่เหลือ (รอเจ้าของตัดสิน)
1. **ไม่มีบัญชี demo ให้ผู้ตรวจ** — login เป็น OTP อีเมลล้วน ผู้ตรวจ Apple เข้าระบบไม่ได้เลย = ตีกลับข้อ 2.1 แน่นอน
   ทางแก้ที่เสนอ: allowlist อีเมลผู้ตรวจ 1 ตัว → คืน OTP ค่าคงที่ (เฉพาะอีเมลนั้น) + seed ร้านตัวอย่างพร้อมข้อมูล
2. **ลบ "บัญชีผู้ใช้" จากในแอปไม่ได้** — มีแต่ลบ "ร้าน" (`requestTenantDeletion`) · Apple 5.1.1(v) บังคับให้ลบ *บัญชี* ได้จากในแอป
   ตอนนี้หน้าเว็บบอกให้เมลมา (จริงตามที่ทำได้) แต่ Apple มักไม่รับ → ต้องทำปุ่มลบบัญชีจริง (โจทย์ที่ต้องเคาะ: ถ้าเป็น OWNER คนสุดท้ายของร้าน ให้ทำยังไง)
