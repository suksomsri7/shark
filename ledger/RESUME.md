...
- มติเจ้าของ 10 ข้อ → `ledger/DESIGN-CRM.md` §9 (HubSpot-style · Team กลาง · คอมมิชชัน PAID · อีเมลตั้งเส้นทางได้ · โทรมือ+AI · web tracking ทำเป็นการตั้งค่า · custom objects ไม่จำกัด · STAFF=ทีม · **portal B2B รอยืนยัน** (อธิบายใน §9))
...
- เมื่อสั่ง "เริ่ม RUN CRM v2": session ใหม่ → worktree นี้ (`pnpm install` แล้ว) → CRM-RUN §0 · C1 เริ่มได้เลย · C2.1 รอสมาชิก M3.3 · C3.5 portal ทำท้ายสุด/ตัดได้
...
- ✅ **เจ้าของเคาะ §8 ครบ (~14:00 BKK)**: REST ทำจริงตาม scope · อันตรายเปิดได้ (scope+confirm) · default bundle ออกเอกสาร+รับเงิน/1 ปี · **E3 (AI ในแอป) ตัดออก → เป็นงานถัดไปหลัง run นี้** · คู่มืออังกฤษหลัก · ⇒ **พร้อมเริ่ม A1 เมื่อเจ้าของสั่งเริ่ม run** · ของเดิมยังรอ: BEAM_* · DBD_API_KEY · inbox@ · ลบ tenant ทดสอบ prod 3 ราย
...
- บั๊กเก่าที่ปิด: expense มี nextDocNo แยกใช้ TZ เครื่อง · convert ไม่คัดลอกแท็ก · saveSettings รีเซ็ต VAT · balanceSheet ไม่เคยได้ปีบัญชี · sweep ปิดงวดทุกระบบไม่มีสวิตช์ · mergePermissions ล้างสิทธิ์โมดูลอื่น · ตัดการเชื่อมแล้วยังลงบัญชี · หน้าเอกสารไม่ดูสิทธิ์
- 🔴 บทเรียนเฟสนี้ (qc:all แดง 3 รอบก่อนผ่าน): regression ที่ด่านเก่าจับ (1.7 ใบพิมพ์กลุ่ม) · default ใหม่เปลี่ยนพฤติกรรมร้านเดิม (autoClose) · ข้อสอบผูก "วันที่ N ของเดือน" ระเบิดเมื่อวันไทยข้าม (1.9 P4.13) ⇒ Fable รันด่านเก่าทุกชุดที่เกี่ยว + fitness แบบไม่มี .env ทุก WO
- 🔑 รอเจ้าของ: crontab เพิ่มบรรทัด `email-reports` (08:30) · `RESEND_API_KEY` บน prod · BEAM_* · DBD_API_KEY · inbox@ email infra
- ถัดไป: เฟส 9 (9.1 มือถือ g17–g20 กำลังทำ → 9.2 audit ความปลอดภัย → 9.3 ประสิทธิภาพ → 9.4 ความง่าย) → เฟส 10 ปิดงาน
...
- 🔴 ส่งต่อ session แชท (หลัง 3.1): คอลัมน์ `ChatContact.partyId` + facade `party.findOrCreate` — บรรทัดที่ต้องแก้ใน `maybeAutoLinkMember` จะอยู่ใน `wo-notes/3.1.md`

## 🏁 3 ก.ย. (ค่ำ) — [session บัญชี] **เฟส 1 ปิด** (WO 1.1–1.9 ขึ้น main `1884f8d` · qc:all 197/197) — สถานะสด `ledger/ACCOUNT-V2-RUN.md`
- ขึ้น prod แล้ว: หน้ารายการทุกชนิด (f3/f13) · ฟอร์มเอกสารเต็มหน้า (g1/g17) · มัดจำ/รับชำระหลายครั้ง/WHT/void (g2) · หน้าเอกสาร (g4/f14) · wizard ลด/เพิ่มหนี้ (g3) · ใบวางบิลรวม/ใบรวมจ่าย · นำเข้า CSV · เอกสารประจำ+แจ้งเตือน (crontab บน VPS: 23:10/01:00 UTC · log /var/log/shark-acc-cron.log)
- บั๊กเก่าที่ปิด 11 ตัว (ร้ายสุด: ใบเสร็จจากใบแจ้งหนี้ลงรายได้+VAT ซ้ำ 2 เท่า · มัดจำจ่ายโพสต์ 2 รอบ · ซื้อราคารวม VAT ไม่สมดุล · VAT รอใบกำกับลงผิดบัญชี)
- 🔴 กติกา: งานหนักทีละ 1 อย่าง · ห้าม `source .env*` · fixture ของ chromium ห้ามอยู่ /tmp (snap) · ทุก WO มี UI ต้องผ่านภาพจริงเทียบ mockup โดย Fable
- ถัดไป: เฟส 2 (2.1 query dashboard กำลังทำ → 2.2 หน้าหลัก f1/f11 → 2.3 ภาพรวมรายรับ/รายจ่าย f4)
...
- แก้: **WAV ผ่าน Web Audio เป็นเส้นทางเดียว** (MediaRecorder เลิกใช้ผลิตไฟล์ — ลบบล็อกทิ้ง) · ฟองดัก `onError` +
  `play().catch` สลับเป็นลิงก์ · **แอปเล่นในแอปผ่าน WebView** (เจ้าของสั่ง "ไม่เด้ง browser" · OTA group `8e8b5055`) ·
  ไฟล์เสีย 2 ไฟล์ transcode ด้วย ffmpeg บน VPS → อัป Bunny path ใหม่ → ชี้ DB (แถวละ 1 ✓ · API v1 ส่งตัวใหม่แล้ว)
- ข้อสอบ: VO-10.1 = "WAV เส้นทางเดียว" · VO-10.5 = "ดัก error ตอนเล่นจริง" · voice 30/30
- งานต่อยอด (คิว): ตัวแปลง wav→m4a ฝั่งเซิร์ฟเวอร์ (ffmpeg — สำหรับ LINE + ลดขนาดไฟล์)
- 🔴 บทเรียน: "ชนิดไฟล์ถูก" ≠ "โครงไฟล์ถูก" (ดู ftyp) · fallback ต้องดักตอนเล่นจริง ไม่ใช่แค่ canPlayType

## 🔊 2 ก.ย. — เจ้าของเทสเสียงจริง → พัง 3 จอ (iPhone) → ราก: iOS เล่น webm ไม่ได้ → แก้ D29
- อาการ: SHARK web กด play เงียบ · เว็บลูกค้า player ไม่เล่น · แอปเป็นกล่องดำ — ไฟล์อยู่บน CDN ครบ (200 · CORS ok)
  แต่เป็น `webm/opus` (ตัวอัดเดิมเลือก webm ก่อนบน Chrome เดสก์ท็อป) ซึ่ง iOS เล่นไม่ได้ทุกทาง
- แก้ (D29 ใน PLAN §10): อัด **m4a เป็นหลัก** · Firefox ตก **WAV ผ่าน Web Audio** (16kHz โมโน) · เลิกผลิต webm/ogg ·
  ฟองทุกฝั่งตรวจ `canPlayType` → ไฟล์เก่าได้ลิงก์เปิดฟังแทนปุ่มเงียบ · storage รับ audio/wav · ข้อสอบ VO-10 คุมถาวร
- แอป (siamdive-rn · สาย J): เรนเดอร์ไฟล์แนบ + เปิดฟังผ่าน Linking — **JS-only ห้ามเพิ่ม native dep** ·
  เล่นในแอปเองต้องเพิ่ม expo-audio ตอน build หน้า · **ยังไม่ OTA/บิลด์ รอเจ้าของ/Fable เคาะช่องทาง**
- ผลพลอยได้: ไฟล์ m4a คือชนิดที่ LINE รับ ⇒ งาน "LINE ส่งเสียง" ขยับใกล้ขึ้น (เหลือแค่เคสไฟล์ wav)
- 🔴 บทเรียน: format ที่ผู้ใช้แลกกันต้องเลือกจากฝั่ง "ผู้เล่นที่แย่ที่สุด" ไม่ใช่ฝั่ง "ผู้อัดที่สะดวกที่สุด" ·
  ข้อสอบ static จับเรื่องนี้ไม่ได้ — ฟีเจอร์สื่อ ต้องเทสด้วยเครื่องจริงต่างค่ายก่อนประกาศเสร็จ
...
1. 🍎 **iOS: ถอนใบยื่นออกจากคิวแล้ว รอเทส build #23** — ปุ่ม Google ทำแอปดับบน #22 (plugin ขาด `iosUrlScheme`)
   แก้แล้ว + build #23 VALID ผูกกับเวอร์ชัน 1.0.0 แล้ว · **เจ้าของกดปุ่ม Google บน TestFlight ให้ผ่านก่อน แล้วค่อยยื่นซ้ำ**
   (ถ้าขึ้น "Access blocked" = OAuth consent screen ยังเป็นโหมด Testing ต้องกด Publish app — คนละเรื่องกับ SHA-1)
   · ขั้นตอนยื่นทั้งหมด: `ledger/SUBMIT-STEPS.md`
2. 🤖 **Android: APK เทสเครื่องพร้อมแล้ว** · ต้องเอา SHA-1 ไปลงทะเบียนใน Google Cloud ก่อนปุ่ม Google จะใช้ได้
   `D9:3A:A1:B0:7E:1A:DB:9E:A3:D7:B6:7F:C9:A7:1A:8C:11:5E:8D:53` · คู่มือ Play ครบใน `ledger/PLAY-STEPS.md`
   (บัญชี Play เป็นแบบบริษัท → ไม่ติดกฎ closed testing 12 คน 14 วัน)
3. 🔑 หลัง iOS ผ่าน review → **ลบ env `REVIEW_EMAIL` / `REVIEW_OTP` บน Vercel**

### งานเว็บที่ปิดไปในรอบนี้ (ทั้งหมด deploy + verify บน prod แล้ว)
- **ตั้งค่าเอกสาร (บัญชี)**: คำนำหน้าชื่อกิจการ (dropdown) · คำแนะนำ URL เว็บ + เติม https:// ให้เอง ·
  โลโก้/ตราประทับ/ลายเซ็น อัปโหลดได้ครบ 3 ช่อง + **ปุ่มลบพื้นหลัง** (canvas ในเครื่อง ไม่ส่งรูปออกนอก)
  · ชื่อบนเอกสารรวมที่ `orgDisplayName()` ที่เดียว (ใบกำกับ · 50 ทวิ · ใบเสร็จสาธารณะ)
- **เมนูบัญชี**: ยกทั้ง 8 หมวด 28 รายการขึ้นไปอยู่ใน ☰ (อ่านทะเบียนเดียวกับ sidebar) · หน้าแรกเหลือ 1 จอ
- **จัดการระบบ/กิจการ**: เพิ่มกิจการได้จริง (`?add=1`) · สร้างระบบแล้วเมนูขึ้นทันที (`revalidatePath layout`)
  · เอาระบบออกได้ (ซ่อน ไม่ลบข้อมูล) ที่ `/app/settings/systems` + ลิงก์ในเมนู
- **แชท**: ลิงก์แชทหน้าเว็บชี้โดเมนที่ปิดแล้ว → `publicOrigin()` เอาโดเมนจากคำขอจริง (+แก้ env APP_URL บน Vercel)
  · เชื่อมระบบสมาชิกอัตโนมัติเมื่อร้านมีชุดเดียว (คอลัมน์ใหม่ `memberSystemChosenAt` แยก "ไม่เคยตั้ง" กับ "ตั้งใจปิด")

### ข้อสอบใหม่ในรอบนี้ (ทุกชุดพิสูจน์ fail-before แล้ว)
`qc-account-doc-settings` 23 · `qc-systems-manage` 12 · `qc-public-links` 11 · `qc-chat-member-autolink` 11
· `qc:appstore` 24→28 · `qc-mobile-app` 35→38 · `qc-nav-functions` ตรวจลิงก์ 71→99
🔴 **ข้อสอบใหม่เข้าเป็นด่าน CI อัตโนมัติตั้งแต่วินาทีที่ไฟล์ถูกสร้าง** (`pnpm qc:all` อ่านโฟลเดอร์เอง)

### บทเรียนของรอบนี้ (อย่าเสียเวลาซ้ำ)
- 🔴 **grep แยกโค้ดกับคอมเมนต์ไม่ออก** — เจอซ้ำอีกครั้งตอนเขียน `qc-public-links` (คอมเมนต์ที่เล่าว่า
  "เดิมเคยใช้ env.APP_URL" ถูกนับเป็นการใช้จริง = ผลลบปลอม 2 ข้อ) → ตัดคอมเมนต์ก่อน grep เสมอ
- 🔴 **chromium บนเครื่องนี้เป็น snap = มี /tmp ส่วนตัว** — `uploadFile("/tmp/x.png")` ในเทส headless
  ทำให้ POST ล้มเป็น `net::ERR_FAILED` โดยไม่มีสาเหตุบอก · วางไฟล์ทดสอบไว้ใต้ `/root/projects/...` แทน
- 🔴 **catch เปล่า ๆ กลืนสาเหตุ** — `storage.uploadFile` ไม่เคยบันทึกว่าล้มเพราะอะไร (FileAsset = 0 แถวมาตลอด)
  ตอนนี้ลง OpsEvent แล้ว · เจอแพตเทิร์นเดียวกันกับ push ที่พังเงียบเพราะ catch เปล่า
- 🔴 **ค่า env ที่ตั้งด้วยมือเน่าเงียบ** — `APP_URL` ค้างเป็นโดเมน VPS ที่ปิดไปแล้ว ทำให้ลิงก์สาธารณะทุกเส้น
  (แชท · ฟอร์ม · เพจ · จอคิว · พอร์ทัลผู้ขาย · **ลิงก์ในอีเมล**) ตายหมดโดยไม่มีอะไรฟ้อง

## 🚀 22 ส.ค. (บ่าย) — build #22 ขึ้น TestFlight + ปิดช่องบังคับของ ASC ครบ
**เหลืออย่างเดียว: App Privacy ในเว็บ (กด Publish ด้วย) แล้วกด Submit** — ขั้นตอนอยู่ `ledger/SUBMIT-STEPS.md`
- **build #22 VALID บน ASC · ผูกเข้าเวอร์ชัน 1.0.0 แล้ว** (ยืนยันด้วย `asc-listing.py show` ไม่ใช่เชื่อ CLI)
  · TestFlight กลุ่ม "ทีมเทส SHARK" (internal · รับทุกบิลด์อัตโนมัติ) → โหลดเทสได้เลย
  · บิลด์นี้พา 3 อย่างขึ้นเครื่อง: ชื่อใต้ไอคอน SHARK HUB · push ที่เคยพังเงียบ · logout ถอนทะเบียน
- 🔴 **เจอว่าช่องบังคับ 4 ชุด "ยังไม่ตอบเลยสักช่อง"** (บันทึกเดิมเข้าใจว่าเหลือแค่ App Privacy + Submit = ไม่ครบ)
  → สคริปต์ใหม่ `scripts/asc-compliance.py show|apply` ตั้งครบแล้ว:
  **Age Rating → Apple ให้เรต 4+** (messagingAndChat=true ตอบตามจริง · userGeneratedContent=false
  เพราะเนื้อหาอยู่ในร้านตัวเอง ไม่เผยแพร่สาธารณะ — ตอบ true จะเข้ากติกา 1.2 ที่บังคับปุ่มรายงาน/บล็อก)
  · **ราคา = ฟรี** · **175 ประเทศ** + รับประเทศใหม่อัตโนมัติ · **Content Rights = ไม่มีของบุคคลที่สาม**
- 🔴 กับดัก API: `limit[territoryAvailabilities]` สูงสุด **50** (ใส่ 200 = error) → ถ้าเหมาว่า error แปลว่า
  "ยังไม่ได้ตั้ง" จะได้ **ผลลบปลอม** (ตั้งสำเร็จแล้วแต่จอบอกยังไม่ตั้ง) · นับจาก `meta.paging.total`
- ⚠️ `eas-cli build:view` **ไม่มีแฟลก `--non-interactive`** (มีใน build/build:list) → ถ้าใส่ คำสั่งตายเงียบ
  ใช้ `build:list --limit 1 --json --non-interactive` แทนเวลาเช็คสถานะแบบอัตโนมัติ

## 🧾 22 ส.ค. — ตรวจซ้ำก่อนยื่น เจอของพัง 3 อย่างที่ไม่มีใครเห็น (main=e7f7ea7)
**ขั้นตอนที่เหลือทั้งหมดย้ายไปอยู่ `ledger/SUBMIT-STEPS.md`** (บิลด์ #22 · คำตอบ App Privacy ทุกช่อง · เช็คลิสต์ก่อนกด Submit)
เขียนเป็นไฟล์เพราะรอบก่อนคำแนะนำอยู่ในหน้าแชทแล้วหายไปพร้อม session ที่ตาย

1. 🔴 **ร้านผู้ตรวจ "รันซ้ำแล้วสด" ไม่จริง** — บิลขาย 4 ใบลงวันที่ 21 ส.ค. ทั้งหมด → ยอดขายวันนี้ ฿0
   เหตุ: กุญแจกันซ้ำของบิลเป็น `seed-review-sale-a` ไม่มีวันที่ (ต่างจากนัดหมายที่มี `dateStr` จึงรีเฟรชเองได้)
   → ใส่ `${dayKey}` แล้วรันบน prod: บิลวันนี้ 4 ใบ ฿2,140 · นัดที่ยังไม่ถึง 6 รายการ
2. 🔴 **push พังเงียบมาตั้งแต่ย้ายบัญชี Expo (20 ส.ค.)** — `push-register.ts` ฮาร์ดโค้ด projectId `c24ec712-…`
   ของบัญชี `@siamdive` เดิม (ของจริง `06c3edc8-…` ใน app.json) → ขอ token ด้วย id ที่ไม่ได้เป็นเจ้าของ
   ลงทะเบียนไม่ติด **และ catch กลืนเงียบ** · แก้เป็นอ่านจาก `Constants.expoConfig.extra.eas`
3. 🔴 **logout ไม่ถอนทะเบียนเครื่อง** — ฝั่ง server รับ `{expoToken}` เพื่อลบ PushDevice มาตั้งแต่แรก
   แต่แอปส่ง body ว่าง → เครื่องที่ออกจากระบบแล้วยังได้แจ้งเตือนของบัญชีเดิมต่อ
   · เพิ่ม `currentPushToken()` (ไม่เด้งขอ permission ตอน logout) + `resetPushRegistration()`
- ทั้ง 2 ข้อของแอปจะติดไปกับ **build #22** พอดี (ยังไม่ได้บิลด์ = ยังไม่เสียของ)
- ด่านใหม่ 6 ข้อ · `qc:appstore` 24→**28** (AS-7.1/7.2 กุญแจต้องมีวันที่ · 7.3 ห้ามวันที่ฮาร์ดโค้ด
  · **AS-7.4 ชื่อใต้ไอคอน (app.json) ต้องเท่ากับชื่อในสโตร์ (asc-listing.py)** — ชื่อ 2 ที่นี้แก้คนละจังหวะ เพี้ยนง่าย)
  · `qc-mobile-app` 35→**37** (APP-7.3 ห้ามพิมพ์ uuid ซ้ำ · APP-7.4 logout ต้องส่ง expoToken)
  ทุกข้อพิสูจน์ fail-before แล้ว (ย้อนไฟล์กลับ → แดงตรงข้อที่ควรแดง)
- `/privacy` เพิ่ม 3 หัวข้อที่ของจริงมีแต่หน้าไม่เคยบอก: ผู้ช่วย AI ส่งข้อความ/รูปไปประมวลผลที่ OpenRouter
  · เก็บ push token · สิ่งที่ไม่เก็บ (ตำแหน่ง/รายชื่อผู้ติดต่อ/บัตรเครดิต/ตัวติดตามโฆษณา) → ตรงกับตาราง App Privacy
- 🔴 **บทเรียน**: "สคริปต์รันซ้ำได้" ≠ "รันซ้ำแล้วข้อมูลสด" — idempotent ที่ไม่มีเวลาอยู่ในกุญแจ
  = แช่แข็งของเก่าไว้ตลอดกาล · ของที่ต้องสดต้องมีวันที่อยู่ในกุญแจ ไม่ใช่แค่กันซ้ำ

## 🍎 21 ส.ค. — หน้าร้าน App Store พร้อมยื่นแล้ว (เหลือ 2 อย่างที่เป็นมือเจ้าของ)
### ชื่อแอป — `SHARK BC` → `SHARK HUB`
- เจ้าของขอ `SHARK` → Apple ปฏิเสธ **409 ชื่อถูกจองแล้ว** · `SHARK AI` ก็โดนจองเหมือนกัน
- ลองไล่ตัวสำรอง → `SHARK POS` ว่าง (ตั้งชั่วคราว) → เจ้าของเลือก **`SHARK HUB`** ✅
- 🔴 **ชื่อมี 2 ที่ อย่าสับสน**: ชื่อในสโตร์ (ASC · แก้ได้ทันที) กับชื่อใต้ไอคอน (`app.json expo.name` · **ต้องบิลด์ใหม่**)
- แก้ `app.json` + `login.tsx` เป็น SHARK HUB แล้ว (`5c40703`) แต่ **build #21 ที่ผูกอยู่ยังเป็นชื่อเก่า**
- ⛔ **build #22 ยังไม่ได้ยิง** — คำสั่ง `eas-cli build` ถูก **classifier ของ Claude Code บล็อก** 2 ครั้ง
  (ไม่ใช่กติกาโปรเจกต์) → เจ้าของต้องรันเอง หรือเพิ่ม Bash permission rule ให้ก่อน

### เครื่องมือใหม่ (ใช้ซ้ำได้ทุกเวอร์ชันที่จะยื่นต่อจากนี้)
- `scripts/asc-listing.py show|apply` — อ่าน/ใส่ข้อมูลหน้าร้านทุกช่อง · ตรวจความยาวก่อนยิง (subtitle 30 · keywords 100 · promo 170 · desc 4000)
- `scripts/asc-screenshots.py [--clear]` — อัปรูป 3 จังหวะตามที่ Apple บังคับ + **อ่านสถานะกลับมาตรวจ** (Apple แจ้ง fail เงียบ ๆ)
- `scripts/shot-appstore.mjs` — ถ่ายจากร้านตัวอย่างบน prod ขนาด **1290×2796** เป๊ะ (viewport 430×932 @ DPR 3)

### 🔴 กับดัก ASC API ที่เสียเวลาไปแล้ว — อย่าพลาดซ้ำ
1. **token อายุเกิน 20 นาที = 401** · ข้อความ error บอกแค่ "credentials missing or invalid" ไม่ได้บอกว่าอายุเกิน
2. **ไม่มี `APP_IPHONE_69`** ใน API (แม้ Apple จะโฆษณาจอ 6.9") — ตัวใหญ่สุดคือ `APP_IPHONE_67` = 1290×2796
3. **เลขเวอร์ชันในสโตร์ต้องตรงกับ `CFBundleShortVersionString` ของ build เป๊ะ** — สโตร์เป็น `1.0` แต่ build เป็น `1.0.0`
   → ผูก build ไม่ได้มาตลอด และ ASC **ไม่บอกเหตุผล** แค่ปล่อยช่อง build ว่างไว้เฉย ๆ

### สถานะช่องต่าง ๆ (ยืนยันด้วยการอ่านกลับจาก ASC ไม่ใช่เชื่อว่าคำสั่งไม่ error)
✅ เวอร์ชัน 1.0.0 · ผูก build #21 · หมวด Business+Productivity · 2 ภาษา **en-US + th** (ชื่อ/subtitle/คำบรรยาย/keywords/URL)
✅ ข้อมูลผู้ตรวจ + บัญชี demo + วิธีหาปุ่มลบบัญชี · รูป 5 ใบ × 2 ภาษา (ถ่ายจากระบบจริง ไม่ใช่ mock)
⛔ **เหลือที่ API ทำไม่ได้**: App Privacy (แบบสอบถาม) + กดปุ่ม Submit — ต้องเป็นเจ้าของทั้งคู่
⚠️ นัดหมายในร้านตัวอย่างเป็นวันสัมพัทธ์ → **ก่อนกด Submit ให้รัน `scripts/seed-review-shop.mts` ซ้ำ** ไม่งั้นปฏิทินว่าง

## 🎨 21 ส.ค. — รื้อหน้า landing (เจ้าของสั่ง 3 ข้อ) + 🔴 พบร้านผู้ตรวจว่างเปล่า
### 🔴 สิ่งที่เจอระหว่างทำ (สำคัญกว่างานที่สั่ง)
**ร้านตัวอย่าง (App Review) บน prod ว่างเปล่า** — สินค้า 0 · พนักงาน 0 · ยอดขาย 0 · นัดหมาย 0
(มีแค่ booking service 3 + ช่าง 1) → **ผู้ตรวจ Apple เปิดแอปแล้วเจอ "ยังไม่มี…" ทุกหน้า = เสี่ยงตีกลับข้อ 2.1**
บันทึกเดิม (20 ส.ค.) เขียนว่าร้านนี้ "มี POS + สินค้า/บริการ + HR" — จริงแค่ว่า **เปิดระบบไว้** แต่ **ไม่มีข้อมูลข้างใน**
→ `scripts/seed-review-shop.mts` (idempotent · resolve ร้านจากชื่อ ไม่ hardcode id · ใช้ service จริงทุกโมดูล)
เติมแล้ว: พนักงาน 4 · สินค้า 6 (รับเข้าจริง มี 1 ตัวใต้จุดสั่งซื้อ→โชว์การ์ด "ใกล้หมด") · ช่าง +2 · นัดหมาย 5 · บิลขาย 4 (฿2,140)

### งานที่สั่ง 3 ข้อ
1. **orb ผู้ช่วย AI กลางหัวหน้าแรก — หมุน + หัวใจเต้น** · ใช้ `.ai-orb` **ตัวเดียวกับปุ่มในแอป** ไม่วาดใหม่
   - `--orb-ring` = ตัวแปรใหม่คุมความหนาวงแหวน (ค่าเริ่ม 8px = ของเดิมในแอป **ห้ามแตะ**) · `.ai-orb-lg` = 14px สำหรับตัวใหญ่
   - `.ai-orb-heartbeat` = ตุบ-ตุบ-พัก 2.6 วิ (คนละจังหวะกับ `.ai-orb-breathe` ของปุ่มในแอปที่เป็นลมหายใจสม่ำเสมอ)
   - `prefers-reduced-motion` ปิดทั้งคู่
2. **ใส่รูป** — `public/shots/*.webp` 4 รูป **ถ่ายจากระบบจริงบน prod** ด้วย `scripts/shot-landing.mjs`
   (login เป็นบัญชีผู้ตรวจ → เดินเส้นทางจริง → ครอป/บีบ webp ~45 KB/รูป) · 🔴 ห้ามแทนด้วยภาพ mock (Apple 2.3)
3. **ไอคอนระบบต้องเหมือนในแอป** — landing เลิกพิมพ์ลิสต์เอง เปลี่ยนเป็นอ่าน `SYSTEM_DEFS` ตรง ๆ
   - ลิสต์เดิม **เพี้ยนจากแอปจริง** (POS ในลิสต์ = 💵 แต่แอปใช้ 🧾) · ตอนนี้เพี้ยนไม่ได้เชิงกลไก
   - การ์ดใช้หน้าตาเดียวกับ `AddSystemModal` ในแอป (`rounded-xl border p-3` + icon/label/hint)
- ข้อสอบ `qc:appstore` **19 → 24 ข้อ**: AS-6.1 เลิกนับ `title: "` (วัดผิดตัว — นับลิสต์ที่พิมพ์มือซึ่งเพี้ยนได้)
  → วัดจากทะเบียนจริง · **AS-6.3 ใหม่** landing ต้องอ่าน SYSTEM_DEFS · **AS-6.4 ใหม่** รูปบน landing ต้องมีไฟล์จริง

## 🕳️ 21 ส.ค. — เปิดด่านครบ 152 ชุด แล้วขุดเจอบั๊กที่ซ่อนใน CI มานาน 3 ตัว
ทั้ง 3 ตัวนี้ **มีอยู่มาตลอด** แต่มองไม่เห็นเพราะ CI เดิมรันแค่ 3 ชุดที่บังเอิญไม่แตะจุดเหล่านี้
1. 🔴 **`SESSION_SECRET` ใน ci.yml ยาว 25 ตัว แต่ `src/lib/env.ts` บังคับ `min(32)`**
   → ทุกชุดที่ import `core/session` **พังตั้งแต่ boot** (zod throw) · แก้เป็น 45 ตัว
2. 🔴 **ไม่ได้ตั้ง `GOOGLE_WEB_CLIENT_ID`** → route google ตอบ `503 google_disabled`
   ข้อสอบ MA-12.2 ("token ปลอมต้องโดน 401") จึง**วัดอะไรไม่ได้เลย** · ใส่ค่าปลอมใน ci.yml ให้เส้นทาง verify ทำงานจริง
3. 🔴 **`apps/mobile` ไม่ได้อยู่ใน pnpm workspace** (มี `package-lock.json` ของ npm แยก)
   `pnpm install` ที่ราก **ไม่ลง** dep ของแอป → APP-5.1 (`tsc --noEmit`) ฟ้อง "Cannot find module 'react-native'"
   → **แก้ที่ตัวข้อสอบ ไม่ใช่ ci.yml**: `qc-mobile-app.mts` `npm ci` เองถ้าไม่มี node_modules
   (ถ้าไปเขียนใน ci.yml ว่าชุดนี้ต้องลง dep ก่อน = กลับไปเป็น "ต้องมีคนจำ" ซึ่งเป็นต้นเหตุเดิมที่เพิ่งแก้)
- วิธีจำลองสภาพ CI บนเครื่อง (ใช้ซ้ำได้): `git worktree add /tmp/ci-repro HEAD` + symlink `node_modules` ราก
  + **ไม่มี `.env`** + ตั้ง `DATABASE_URL/DIRECT_URL` ชี้ Neon branch + `SESSION_SECRET` ≥32
- 🔴 **บทเรียนเครื่องมือ**: `export A=x B=$A` บรรทัดเดียว bash ขยาย `$A` **ก่อน** กำหนดค่า → ตัวแปรว่าง
  เสียเวลาไปกับผลลัพธ์ปลอม "แดง 19 ชุด" · จับได้เพราะเช็คด้วยตัวคุม (`${#DATABASE_URL}` ต้องไม่ใช่ 0)
- ยืนยันก่อน push: รันครบ 6 ส่วนในสภาพจำลอง CI = **26+26+25+25+25+25 = 152/152 เขียวหมด**

## 🔴 21 ส.ค. — บทเรียน: อย่าประเมินเวลา CI จากเวลาเครื่อง dev (latency-bound)
รอบแรกผูก `qc:all` เข้า CI แบบรันงานเดียว timeout 45 นาที (ประเมินจากเครื่อง dev 8 นาที เผื่อ 5 เท่า)
→ **ถูกตัดจบเปล่า 2 รอบ** (run `32442205050` / `32444463505`) — ไม่ได้แดง ไม่ได้เขียว คือไม่มีผลเลย
**วัดจริง: runner ช้ากว่าเครื่อง dev ~10 เท่า** — `qc:account` 18 วิ บนเครื่อง → **195 วิ บน CI**
เหตุ: DB อยู่ `ap-southeast-1` แต่ runner อยู่ US → ทุก round-trip ~200ms · ข้อสอบยิง query ต่อเนื่องเป็นร้อยครั้ง
→ 152 ชุดเรียงกัน ≈ **92 นาที** · แก้เป็น **ซอย 6 ส่วนรันขนาน** `pnpm qc:all --shard=i/6`
- แบ่งแบบสลับฟันปลา (`idx % n`) ไม่ใช่ตัดเป็นก้อน — ชุดชื่อใกล้กันมักหนักพอ ๆ กัน ตัดเป็นก้อนจะได้คอขวด
- พิสูจน์ก่อน push: 6 ส่วน = 26+26+25+25+25+25 = **152 ไม่ซ้ำไม่ขาด** · ส่วนที่ 3 รันจริง 25/25 ใน 147s
- `fail-fast: false` — อยากเห็นทุกส่วนที่แดงในรอบเดียว ไม่ใช่หยุดที่ตัวแรก
- 1 ส่วน = 1 Neon branch (`ci-<runid>-s<n>`) · Neon compute-time รวมเท่าเดิม · runner ฟรี (รีโป public)
- ⚠️ `concurrency.cancel-in-progress: true` → push ถี่ ๆ จะยกเลิกรอบก่อนหน้า **"cancelled" ไม่ได้แปลว่าพัง** ให้ดูรอบล่าสุด
