...
4. **ชื่อ migration ไม่ตรงกัน**: พิมพ์เขียว §4.6 ให้ `c` = Visibility/Quota/Commission/Portal แต่ CRM-RUN ให้ `c` = `PosSale.dealId` และ `d` = ชุดนั้น → ยึดชุดใหม่: `a` (C1) · `b` (C2 ทั้งเฟส รวม `PosSale.dealId`) · `c` (C3 ทั้งเฟส)
...
| # | ความเสี่ยง | ตัวกัน |
|---|---|---|
| R1 | migration เปลี่ยน unique ของ `MemberSection/MemberField` บน prod ที่มีข้อมูลสมาชิกจริง | ทำ 2 จังหวะ: เพิ่มคอลัมน์ `objectKey` default `customer` + unique ใหม่ **ก่อน** แล้วค่อยถอด unique เก่าใน migration ถัดไปหลังโค้ดใหม่ขึ้นแล้ว · ซ้อม migrate บนสำเนา prod (Neon branch) ก่อน |
