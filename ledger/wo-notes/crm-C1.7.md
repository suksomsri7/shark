...
## 3.2 D12 — push/deploy
✅ push `90d3043` (+ `36bacdc`) → session/crm + main · deploy ใหม่ `dpl_BqgKn…` ขึ้นจริงหลัง 390 วิ · health `{ok:true,db:true,outboxPending:0}` · ไม่มี migration · ร้าน uiVersion 1: การมองเห็นไม่ทำงาน (probe B1) · ลิงก์ "ทีมขาย" เห็นเฉพาะเจ้าของ/ผู้มีสิทธิ์ ⇒ เจ้าของร้านจริงจะเห็นลิงก์ใหม่นี้ในเมนูตั้งค่า (หน้าจัดทีมใช้ได้กับร้าน v1 ด้วย — ทีมเป็นแกนกลาง)
