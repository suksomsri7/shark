...
**ภาพจริง** — `pnpm exec tsx scripts/visual-kanban.mts 2.12` (ถ่ายซ้ำรอบสองหลังแก้ parity gap) → 3 ใบ ใน
`.qc-shots/kanban/2.12/` failures 0
- `table-bulk-selected-desktop.png` — ติ๊ก 2 แถวแรก: แถบฟ้าอ่อนอยู่บนหัวตาราง (ไม่ใช่ pill ลอยดำล่างจอ) ตรง
  ...
  ตั้งกำหนดส่ง · เก็บเข้าคลัง) · **ช่อง "กำหนดส่ง" เป็นชิปสัมพัทธ์แล้ว** ("ไม่กำหนด" · "1 ต.ค." · "30 ก.ย." ฯลฯ —
  แถวที่เห็นในวิวพอร์ตนี้ล้วนไกลกว่า 6 วันจากวันนี้จริง (7 ก.ย.) จึงเป็นโทนเทาล้วน ยังไม่เห็นสีส้ม/แดงในภาพนี้)
  · คอลัมน์ "แก้ไขล่าสุด" แสดง "6 นาทีที่แล้ว" (สัมพัทธ์)
...
**ภาพยืนยันสีเพิ่ม (นอกสเปกทางการ — ใช้ตรวจตาเอง)** — `.qc-shots/kanban/path/custom-desktop.png`
(`?view=table&due=overdue` บนบอร์ดป่าตอง): 4 แถวที่เลยกำหนดจริง (`#18`–`#21`) แสดงชิป **"เลย 2 วัน" / "เลย 3
วัน" / "เลย 4 วัน" / "เลย 5 วัน" กรอบ+ตัวหนังสือสีแดง** ยืนยันว่า `tone`/`DUE_STYLE` (ของเดิม ไม่ได้แก้รอบนี้)
ทำงานถูกต้องคู่กับ `chipText` ใหม่ — โทนอำพัน (วันนี้/พรุ่งนี้) ใช้กลไกเดียวกันเป๊ะแต่ไม่มีการ์ดตรงเงื่อนไขในชุด
ข้อมูล ณ วันที่ทดสอบจริง (7 ก.ย. 69) จึงไม่มีภาพตัวอย่างสีส้มให้ดู

## 6. ข้อแย้ง oracle — K2.12-S3.3 (Fable ตรวจรับแล้ว: **ข้อแย้งถูกต้อง** — แก้ oracle ให้เทียบด้วย `now` จริง
แทน `NOW` ของ seed → 13/13 ตามที่บันทึกไว้ใน §5 · เก็บหลักฐานเดิมไว้ด้านล่างเป็นบันทึกอ้างอิง)
...
2. ความเห็นที่กฎอัตโนมัติเขียน (`automationRuleId` ไม่ null) ยัง "แก้ไข/ลบได้" ตามเงื่อนไขเดิม
   ...
   เป็นหนี้เผื่อ P3
3. ~~K2.12-S3.3 (ข้อแย้ง oracle §6)~~ — **ปิดแล้ว** Fable แก้ oracle แล้ว (13/13)
4. ภาพชิปสีอำพัน ("วันนี้"/"พรุ่งนี้") ยังไม่มีตัวอย่างจริงในชุดข้อมูล QC ปัจจุบัน (ไม่มีการ์ดที่ครบกำหนดภายใน
   24 ชม.จากเวลาจริง ณ วันที่ทดสอบ) — กลไกเดียวกับสีแดงที่พิสูจน์แล้ว (§6) ไม่ใช่บั๊ก แค่ไม่มีข้อมูลตัวอย่าง
...
## 9. รอบตรวจรับที่ 2 (Fable) — สรุปปิดงาน

1. ข้อแย้ง `K2.12-S3.3` ถูกต้อง — Fable แก้ oracle แล้ว
2. Parity gap เพิ่มเติมที่ Fable เจอ: ช่อง "กำหนดส่ง" ในตารางยังเป็นวันที่เต็มสีเทาทุกแถว (ไม่ใช่ชิปสัมพัทธ์+สี
   ตามความเร่งแบบการ์ดบนบอร์ด) — แก้แล้ว: เพิ่ม `ThaiDatePicker.chipText` (เสริม ไม่กระทบผู้เรียกเดิม) +
   ส่ง `chipText={due?.text}` จาก `TableRowView` (คลิกชิปยังเปิด `ThaiDatePicker` แก้ไขได้ตามเดิม แก้เสร็จกลับ
   มาเป็นชิปสัมพัทธ์) — พิสูจน์ด้วยภาพจริง 2 ชุด (ตารางปกติ + กรอง `due=overdue` เห็นชิปแดง "เลย n วัน")
3. ผลด่านสุดท้าย: oracle 13/13 · `qc-kanban-k2.1.mts` 22/22 · tsc 0 error · ภาพ spec `2.12` ถ่ายซ้ำ 3 ใบ
   (ดูด้วยตาแล้ว) + ภาพยืนยันสีเพิ่ม 1 ใบ (นอกสเปก) — ไม่ได้รัน fitness/regressions ทั้งชุดซ้ำในรอบนี้ตามที่
   Fable สั่งเจาะจงแค่ 2 ไฟล์ (k2.1/k2.12) เพราะการแก้แตะเฉพาะ `ThaiDatePicker.tsx`+`TableView.tsx` และ
   `qc-kanban-k1.15.mts`/fitness F13 ไม่เกี่ยวกับ prop UI นี้เลย
4. ไม่มี commit ตามที่สั่ง
