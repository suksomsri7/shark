...
## เรื่องที่ Fable ควรรู้เพิ่ม (ไม่ใช่บั๊กของโค้ด — เป็นเรื่องชุดข้อมูล QC)

**`finally` ของ `qc-kanban-k2.9.mts` ทิ้งขยะไว้บนชุด seed 2 อย่าง** (เจอตอนถ่ายภาพ — ไม่ทำให้ข้อสอบชุดไหนแดง แต่ทำให้ภาพ QC ของ WO ถัดไปมีป้ายแปลกปลอม)

1. การ์ด seed `BCD Aqualung ตัวที่ 4 ปุ่มเติมลมค้าง` (บอร์ดซ่อมบำรุง · `due: 2`) **ตรงเงื่อนไขของกฎ DUE_DATE ที่ข้อสอบ S8 สร้าง** ⇒ ถูกติดป้าย `ด่วน-XXXX` ไปด้วยทุกครั้งที่รันข้อสอบ (นี่คือเหตุผลเดียวกับที่ `sweepDueDateRules` ต้องนับเป็น "จำนวนกฎ" — ดู Deviation ข้อ 1)
2. `finally` ลบแถว `KanbanLabel` (join row หายตาม cascade) แต่ **ไม่ได้ sync `KanbanCard.labels` (Json) ของการ์ดที่ไม่ได้อยู่ใน `madeCards`** ⇒ ชื่อป้ายที่ตายแล้วค้างอยู่ใน Json และยังเรนเดอร์เป็นชิปบนการ์ด · รอบหนึ่งยังเจอ `KanbanLabel` ตกค้างจริง 1 แถว (`ด่วน-3720`) ด้วย

ทำความสะอาดคืนให้แล้วด้วยมือ (ลบป้ายตกค้าง + sync `labels` Json ให้ตรงกับแถวจริงทุกใบในร้าน QC) — ตรวจซ้ำหลังรันข้อสอบรอบสุดท้ายแล้วว่าเหลือ 0
เสนอให้เติมใน `finally` ของข้อสอบ: หลังลบป้าย ให้ `kanbanCard.update` ค่า `labels` ของการ์ดที่เคยผูกกับป้ายที่ลบ (หรือเลือกวันครบกำหนดของ `cardE` ที่ไม่ชนกับ seed)
