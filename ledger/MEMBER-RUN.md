...
| M3.1 | ✅ DONE 20/20 · ภาพ 21 ขั้น 1 ผ่าน (ตีกลับ 1 รอบ: client component import โมดูล prisma → build พัง) · Fable แก้ข้อสอบ S2.6/S2.9/S2.11 คิดสดจาก DB · migration member_v2_g + FK 5 เส้น (prod orphan PointBalance 44 แถวของ tenant ทดสอบที่ถูกลบ · ปลอดภัย) · seed ตรึงระดับหลังระบายคิว (m1.5 กลับเขียว) · qc-all ตัด m1.1 ออก | 11 ก.ย. | `HASH_M31` | `scripts/qc-member-m3.1.mts` · migration `member_v2_g` (MemberSegment) · engine ที่ member/segments.ts · marketing/segments.ts re-export |
