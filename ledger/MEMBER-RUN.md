...
| M2.9 | ✅ DONE 22/22 · ภาพ 09 ผ่าน (ตีกลับ 1 รอบ: โปรไฟล์โชว์ id ดิบ) · migration f2 + f2_identity_fk (FK MemberChannelIdentity→Customer · prod 0 แถว ตรวจแล้ว) · มติ: คำขอลบลูกค้า = ERASE PENDING เสมอ · token บัตร HMAC · reuse 15 นาที · หนี้: SMS gateway ยังไม่มี · LINE_LIFF_ID ยังไม่ตั้ง | 11 ก.ย. | `HASH_M29` | `scripts/qc-member-m2.9.mts` · migration `member_v2_f2` (CustomerSession/CustomerOtp/cardToken — `g` สงวนให้ M3.1/M3.2 ตามตาราง WO) · customer-session.ts + me.ts · /m/[slug]/* |
