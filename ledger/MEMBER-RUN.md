...
| M3.3 | ⏳ oracle พร้อม (30 ข้อ) + สเปคภาพ 07/22 (TMP33) | — | — | `scripts/qc-member-m3.3.mts` · migration `member_v2_g3` · journeys.ts + journey-presets.ts (6 preset) · AutomationRun +customerId/scheduledAt/payload · สถานะ WAITING/HOLDOUT/SKIPPED/CANCELLED |
...
| M3.8 | ⏳ oracle พร้อม (18 ข้อ) + สเปคภาพ 25 | — | — | `scripts/qc-member-m3.8.mts` · ไม่มี migration · reports.ts 7 ชุด (RFM ควินไทล์ + กติกาจัดกลุ่มในหัวข้อสอบ · หนี้สิน = outstanding × burnRateSatang) · CSV BOM · schedule ใน settings + cron memberReportsEmail · /member/reports?tab= |
| M3.9 | ⏳ oracle พร้อม (21 ข้อ) + สเปคภาพ 03 (เทมเพลต) | — | — | `scripts/qc-member-m3.9.mts` · templates/ 17 ไฟล์ (ข้อมูลล้วน) · templates-service.ts validate/preview/apply (fields+tiers+stamps+journeys · idempotent) · FieldDesigner แผงตัวอย่าง |
| M3.10 | ⏳ oracle พร้อม (22 ข้อ) + สเปคภาพ 27 (TMP310) | — | — | `scripts/qc-member-m3.10.mts` · ops ชุดสาม ~72 (segments/campaigns/journeys/reviews/referrals/notifications/reports/settings/api-keys/webhooks/join public lane/me) · test id M3.10-S3.x (F13.7) · webhooks REST + HMAC + retry · join.ts service · หน้า /member/assistant (proposal flow) |
| M3.11 | ⏳ oracle พร้อม (16 ข้อ) + สเปคภาพ 29 (TMP311 · --user customer สำหรับ done) | — | — | `scripts/qc-member-m3.11.mts` · /m/[slug]/join 3 ขั้น + /join/done · /api/mobile/member/{search,scan,summary,stamp} (mobile session) · จอแอป 3 + apps/mobile/qc/shoot-member.mjs (Fable รัน) · สะพาน push ใน /m layout · ห้ามบิลด์/OTA |
