...
> 🔴 **18 ก.ย. 2569**: แผนงานถูกครอบด้วย `ledger/CRM-MASTER-PLAN.md` (ขอบเขต = ทั้ง RUN ไม่ใช่แค่ C1 · เริ่มที่เฟส C0 · โหมด Codex ดู MASTER-PLAN §11.6) · กติกาโค้ด §3 ของไฟล์นี้ยังใช้ทั้งหมด · **เพิ่ม**: ทุกคำสั่ง tsx/tsc/build ต้องผ่าน `bash scripts/iso.sh` (เครื่องฆ่า session เมื่อหน่วยความจำเกิน 5 GB) และข้อสอบที่เขียนฐาน QC ต้องรันทีละชุดผ่าน `scripts/with-gate-lock.sh`
