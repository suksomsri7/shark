...
export type ApiActor = {
  ...
  userId?: string | null;
  /**
   * 🔴 M2.10 — คำขอนี้มาจาก **ตัวลูกค้าเอง** (session ของหน้า `/m/*` ไม่ใช่คีย์ของร้าน)
   * มีค่าเมื่อโมดูลเปิด "เลนลูกค้า" ผ่าน `ApiModuleConfig.altAuth` เท่านั้น · โมดูลอื่นไม่มีวันเห็นค่านี้
   * ห้ามใช้แทน `userId`: ลูกค้าไม่ใช่ผู้ใช้ของร้าน (ไม่มี Membership · ไม่มีสิทธิ์ของพนักงานเลย)
   */
