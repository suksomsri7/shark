// respond.ts — ซองจดหมายกลางของ REST ทุกโมดูล (ยกเป็นของกลางตอน K1.15): สำเร็จ · ผิดพลาด · แปลง error ไทยเป็นรหัส
//
...
import { GENERIC_ACTION_FAILED, isSafeUserMessage, safeReason } from "@/lib/core/errors";

/**
 * รหัส error ทั้งหมดของ REST — คู่มือ/OpenAPI/สกิล AI ของทุกโมดูลอ้างรายการนี้
...
export const API_ERROR_CODES = [
  ...
  // M2.10 — ตรงกันข้ามกับตัวบน: **ลูกค้า** ถือ session อยู่จริง แต่ไปเรียก op ของร้าน
  // (อ่านแต้มของสมาชิกคนอื่น · ไล่รายชื่อสมาชิกทั้งร้าน) ⇒ 403 ไม่ใช่ 401 เพราะการล็อกอินใหม่ไม่ช่วย
  // และต้องไม่ใช่ `scope_missing` ซึ่งจะชวนให้ผู้เรียกไปไล่เติม scope ทั้งที่ไม่มี scope ไหนเปิดทางนี้
  "customer_scope",
  "not_found",
  "method_not_allowed",
  "rate_limited",
...
export const ENVELOPE = Symbol("shark.api.envelope");
...
const GENERIC_TH = GENERIC_ACTION_FAILED;
const GENERIC_EN = "The request could not be processed.";

/** status ที่โมดูลประกาศเองได้ → รหัสในซอง (ตัวอื่นให้ตกไปทางเดาจากข้อความเหมือนเดิม) */
const STATUS_CODE: Record<number, ApiErrorCode | undefined> = {
  403: "forbidden",
  404: "not_found",
  409: "state_conflict",
};

const STATUS_EN: Record<number, string | undefined> = {
  403: "This API key is not allowed to do that.",
  404: "The requested resource was not found.",
  409: "The record is not in a state that allows this operation.",
};
...
export function mapError(e: unknown): MappedError {
  ...
  // error ของโมดูลที่ประกาศ `status` ของตัวเองมาแล้ว (เช่น `KanbanNotFoundError` 404 /
  // `KanbanForbiddenError` 403) — เชื่อค่าที่มันบอก ไม่ต้องเดาจากคำในข้อความ
  // 🔴 เชื่อเฉพาะเมื่อข้อความ "ปลอดภัย" (ไทย) เท่านั้น ⇒ error ของไลบรารีนอกที่บังเอิญมีฟิลด์ชื่อ
  //    เดียวกันจะไม่ถูกส่งข้อความดิบออกไป
  const declaredStatus = typeof e === "object" && e !== null ? (e as { status?: unknown }).status : undefined;
  if (typeof declaredStatus === "number" && isSafeUserMessage(raw) && STATUS_CODE[declaredStatus]) {
    return {
      status: declaredStatus,
      code: STATUS_CODE[declaredStatus]!,
      message_th: raw,
      message_en: STATUS_EN[declaredStatus]!,
    };
  }
