...
export type ApiOp = {
  ...
  /**
   * (M3.10) op นี้ **เป็นไฟล์ CSV เสมอ** ไม่ต้องขอด้วย `Accept: text/csv` — ใช้กับ path ที่ชื่อบอกอยู่แล้วว่าเป็นไฟล์
   * (เช่น `GET /reports/{tab}/csv` ของระบบสมาชิก: ลิงก์ดาวน์โหลดในอีเมล/สเปรดชีตเปิดตรง ๆ ไม่มีทางตั้งหัว Accept)
   * ต้องมี `csv` คู่กันเสมอ · ไม่ประกาศ = พฤติกรรมเดิมทุกประการ
   */
  csvAlways?: boolean;
  /**
   ...
   *   `"optional"` — (M3.10) ส่ง `Idempotency-Key` มา = กันซ้ำแบบ `"request"` · **ไม่ส่ง = ทำงานเลย ไม่เก็บผล**
   *                 ใช้กับเลนสาธารณะที่ผู้เรียกคือเบราว์เซอร์ของลูกค้า (สมัครสมาชิกผ่านลิงก์ของร้าน)
   *                 ซึ่งชั้นบริการกันซ้ำเองอยู่แล้ว (OTP ใช้ครั้งเดียว · ตั๋วสมัครใช้ครั้งเดียว · เบอร์ซ้ำ = คนเดิม)
   *                 🔴 ห้ามใช้กับ op ของคีย์ API — การไม่บังคับ header คือการยอมให้ retry ทำงานซ้ำ
   */
  idempotency?: "request" | "key" | "optional";
  /**
   * ค่าลับที่ op นี้คืน **ครั้งเดียวเท่านั้น** (M2.10 — PIN ของบัตรกำนัล)
   * ตอนตอบซ้ำจากแถวกันซ้ำ คีย์เหล่านี้ถูกแทนด้วย `null` ใน `data`
   *
   * 🔴 ทำไมไม่ปล่อยให้ replay คืนของเดิมทั้งดุ้น: สัญญาของ PIN คือ "เห็นครั้งเดียว ระบบไม่เก็บของดิบ"
   *    ถ้าใครยิงคีย์กันซ้ำใบเดิมซ้ำได้ PIN กลับมาเรื่อย ๆ สัญญานั้นก็เป็นโมฆะ — และชั้นบริการเอง
   *    (`giftcard.sell`) ก็คืน `pin: null` เมื่อยิงซ้ำอยู่แล้ว ⇒ สองชั้นต้องพูดตรงกัน
   */
  replaySecrets?: readonly string[];
