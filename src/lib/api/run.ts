// run.ts — "รัน op หนึ่งตัวในนามของ actor" (ยกเป็นของกลางตอน K1.15)
//
// ทำไมต้องแยกออกมาจาก `dispatch.ts`: ตั้งแต่ WO E1 มีผู้เรียก 2 ทางที่ต้องเดินด่านเดียวกันเป๊ะ
//   1) REST  `/api/v1/<module>/*`      → dispatch.ts (HTTP: header/สถานะ/ซอง/กันซ้ำ)
//   2) สกิล AI `<module>_*`            → src/lib/ai/tools-{account,kanban}.ts (อ่านทันที) + proposals.ts (เขียนหลังคนยืนยัน)
