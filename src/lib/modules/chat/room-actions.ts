...
export async function shopLocationAction(
  ...
  if (link === "" && address === "") {
    return {
      ok: false,
      reason:
        "ยังไม่ได้บันทึกที่อยู่หรือพิกัดของสาขานี้ — ไปที่ ตั้งค่า → จัดการระบบ → ตั้งค่าสาขา แล้วกรอกที่อยู่หรือลิงก์แผนที่ จากนั้นปุ่มนี้จะส่งแผนที่ให้ลูกค้าได้ทันที",
    };
  }
