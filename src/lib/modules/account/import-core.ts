// ─────────────────────────────────────────────────────────────
// import-core.ts — แกนตรวจ/นำเข้า CSV (แยกจาก `import-actions.ts` ใน WO D3)
//
// 🔴 ทำไมแยกไฟล์: `import-actions.ts` มี `"use server"` + import `./guard` (`loadAccountSystem` →
//    `requireTenant` → session/env ตอนโหลดโมดูล) — โมดูลนั้นใช้ได้เฉพาะที่มี request context ของ Next
//    (cookie จริง) เท่านั้น REST บัญชี (`api/ops/import.ts`) เรียกด้วยคีย์ API ไม่มี session แบบนั้น
//    ⇒ `previewImportCore`/`runImportCore` (อาร์กิวเมนต์ธรรมดาอยู่แล้ว ไม่ต้องมี wrapper ใหม่) ย้ายมาไว้
//    ที่นี่ ซึ่ง**ไม่แตะ `./guard`/session เลย** → import โมดูลนี้เฉย ๆ ไม่พังในสภาพแวดล้อมที่ไม่มี .env
//    (fitness F10.1 · บทเรียนเดียวกับ B2: `contacts-read.ts` เคย import แบบลาก session มาด้วย)
// 🔴 ไฟล์นี้ **ไม่ import prisma** โดยเจตนาเช่นกัน — ทุกการแตะ DB ผ่าน service/product/expense/coa (fitness F5)
// `import-actions.ts` (server actions ที่ผูก session) ยัง re-export ทั้งคู่จากที่นี่ให้ผู้เรียกเดิมใช้ได้เป๊ะ
// ─────────────────────────────────────────────────────────────
...
//
// 🔵 แยก action (ผูก request/cookie ผ่าน loadAccountSystem — อยู่ใน import-actions.ts) ออกจาก core
//    (รับ tenantId/systemId ตรง ๆ) ให้เหมือน service.ts/expense.ts/product.ts — QC script
//    (scripts/qc-acc-v2-import.mts) และ REST บัญชี (api/ops/import.ts) เรียกตัวนี้ตรงได้โดยไม่ต้องมี
//    Next.js request context (requireTenant อ่านคุกกี้ผ่าน next/headers ใช้นอก request ไม่ได้)
// ─────────────────────────────────────────────────────────────
