...
import { bulkDecideLeaveAction, type BulkLeaveState } from "./actions";

// เลือกใบลาหลายใบ (checkbox) แล้วอนุมัติ/ปฏิเสธพร้อมกัน — สรุปผลแสดง inline
// ฝังในหน้า HR ส่วน "ใบลารออนุมัติ" (เลือก 1 ใบ = ทำรายเดียวได้)
type Item = { id: string; label: string; meta: string };

export default function BulkLeaveApprovals({ systemId, items }: { systemId: string; items: Item[] }) {
  const [state, formAction, pending] = useActionState<BulkLeaveState, FormData>(
    async (prev, formData) => bulkDecideLeaveAction(systemId, prev, formData),
    { status: "idle" },
  );
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ไม่อนุมัติที่เลือก ({selected.size})
        </button>
      </div>
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold">
              {confirm === "APPROVED" ? "อนุมัติใบลาที่เลือก?" : "ไม่อนุมัติใบลาที่เลือก?"}
            </h2>
            <p className="mt-1 text-sm text-[color:var(--color-muted)]">เลือกไว้ {selected.size} ใบ</p>
      ...
      {state.status === "done" && (
        <div className="rounded-lg border px-3 py-2 text-sm">
          <div className="font-medium">
            สำเร็จ {state.done} ใบ
            {state.failed.length > 0 ? ` · ล้มเหลว ${state.failed.length} ใบ` : ""}
          </div>
          {state.failed.map((f) => (
