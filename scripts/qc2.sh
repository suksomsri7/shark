...
#   Own gate lock (/tmp/shark-gate-qc2.lock) — QC1 and QC2 suites run in parallel; each worktree stays on ONE branch
...
for U in "$D" "$P"; do
  ...
  case "$U" in *ep-cool-shadow*) : ;; *) echo "🔴 qc2: URL is not QC2 (ep-cool-shadow)" >&2; exit 4 ;; esac
done
exec env DATABASE_URL="$P" DIRECT_URL="$D" QC_ENV_FILE="$F" GATE_LOCK_FILE=/tmp/shark-gate-qc2.lock QC_BRANCH=qc2 "$@"
