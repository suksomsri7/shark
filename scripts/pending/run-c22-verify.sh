#!/usr/bin/env bash
# C2.2 controller verification (Fable · 24 ก.ย.) — main tree = C2.1 accepted (d3e58043) + C2.2 patch applied (32 files, staged) · QC1
# BUILD step also takes the QC3 lock so it never runs beside a QC3 oracle from the c23 agents (2 cores / 7 GB).
set -uo pipefail
cd /root/projects/shark-crm
L=.qc-shots/crm/c22-verify.log; : > "$L"
...
q qc-crm-c2.2
for s in qc-crm-c2.1 qc-crm-c0.5 qc-member-fix-s3 qc-automation qc-crm-c1.4 qc-crm-c1.6 qc-crm-c1.8 qc-crm-c1.11 qc-crm-c2.0 qc-crm-v1 qc-crm-c0.2; do q "$s"; done
