#!/usr/bin/env bash
# C0.1 final controller verification — reseed → seed idempotency → DRAIN → m1.9 → oracles → gates → shots
set -uo pipefail
cd /root/projects/shark-crm
L=.qc-shots/crm/c01-final.log
: > "$L"
...
r "DRAIN (qc-cron)"  bash scripts/with-gate-lock.sh pnpm exec tsx scripts/qc-cron.mts
r "m1.9 AFTER drain" bash scripts/with-gate-lock.sh pnpm exec tsx scripts/qc-member-m1.9.mts
