...
//   reportBySource(ctx, actor, { from, to, unitId? }) → { rows: [{ source, sourceChannel, signups, firstTouch, lastTouch, buyers, repeatBuyers, avgSpentSatang, costSatang, costPerSignupSatang }], links: [{ id, code, name, source, hits, signups, firstPurchases, costSatang, costPerSignupSatang }], total } (member.report.view · นับ FIRST เป็นหลัก · signups = สมาชิกที่ createdAt ในช่วง)
...
try {
  ...
  chk("M1.8-S3.1", "reportBySource(1 ปี): rows LIFF {signups 2, firstTouch 2, lastTouch 1 (c2 — c1 ย้าย LAST ไป POS), costSatang 200000 (ลิงก์ A), costPerSignupSatang 100000} · WALK_IN signups 60 · REFERRAL 1 · links มี A {hits 2, signups 2, firstPurchases 1, costPerSignupSatang 100000} · total = Σ signups", liff?.signups === 2 && liff.firstTouch === 2 && liff.lastTouch === 1 && liff.costSatang === 200_000 && liff.costPerSignupSatang === 100_000 && walk?.signups === 60 && refRow?.signups === 1 && lA?.hits === 2 && lA.signups === 2 && lA.firstPurchases === 1 && lA.costPerSignupSatang === 100_000 && rep.total === rep.rows.reduce((s: number, x: Any) => s + x.signups, 0), "ตัวเลขตรง", JSON.stringify({ liff, walk: walk?.signups, ref: refRow?.signups, lA, total: rep?.total }).slice(0, 300));
  ...
  chk("M1.8-S3.2", "unitId=กะตะ → นับเฉพาะ FIRST.unitId กะตะ (WALK_IN 20 · LIFF 0) · ช่วงปี 2020 → total 0 · thana (ไม่มี member.report.view) → throw", repU?.rows?.find((x: Any) => x.source === "WALK_IN")?.signups === 20 && (repU.rows.find((x: Any) => x.source === "LIFF")?.signups ?? 0) === 0 && repOld?.total === 0 && thai(eRepT), "20 · 0 · throw", JSON.stringify({ kata: repU?.rows?.map((x: Any) => [x.source, x.signups]), old: repOld?.total, t: !!eRepT }).slice(0, 200));
