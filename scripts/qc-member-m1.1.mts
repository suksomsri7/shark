...
const hasIdx = async (t: string, colsIn: string[], unique = false) => (await q(`select 1 from pg_indexes where tablename='${t}' ${unique ? "and indexdef ilike '%unique%'" : "and indexdef not ilike '%unique%'"} ${colsIn.map((c) => `and indexdef ilike '%"${c}"%'`).join(" ")}`)).length >= 1;
