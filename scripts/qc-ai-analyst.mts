...
try {
  ...
  if (tid) { for (const m of ["appNotification", "aiUsage", "aiUsageWindow", "aiSettings", "aiCreditTxn", "aiCreditWallet", "posSale", "customer", "appSystemUnit", "appSystem", "businessUnit"]) await d(() => (prisma as never as Record<string, { deleteMany: (a: unknown) => Promise<unknown> }>)[m].deleteMany({ where: { tenantId: tid } })); await d(() => prisma.tenant.delete({ where: { id: tid } })); }
