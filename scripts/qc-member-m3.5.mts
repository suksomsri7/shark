...
//
// สัญญา (ORACLE-EDIT M3.5-S1.1: ตาราง ReferralProgram/Referral ไม่เคยถูกสร้างใน M1.1 — ใบนี้มี migration `member_v2_h3` (20261030000000 · additive · ผู้คุมงานตัดสิน 11 ก.ย.) · src/lib/modules/member/{referrals.ts, referrals-actions.ts} · facade member/index.ts · หน้า /member/referrals · การ์ดแนะนำเพื่อนใน 360 ?tab=referrals · LIFF /m/[slug]/referral (แชร์) · /r/[code] (ทางเข้าเพื่อน → LIFF join ?ref=<code>))
...
//   codeFor(ctx, customerId) → { code, link: "/ref/<code>" (ORACLE-EDIT M3.5-S1.2: /r/[token] = หน้าใบเสร็จสาธารณะของบัญชีอยู่แล้ว · next build ล้ม "Ambiguous route /r/[*]"), shareText (แทน {link} เป็น URL เต็มจาก origin) } · code = Customer.referralCode (M1.4 สร้างให้แล้ว · ว่าง → สร้าง unique 8 ตัว)
...
try {
  ...
  // ORACLE-EDIT M3.5-S1.1: ค่าปริยายต้องวัดตอน "ยังไม่มีแถว" — ถ้ามีแถวค้าง (harness/รอบก่อน) ลบก่อน แล้ว restore คืนตอนจบ (prog0) · ไม่ผ่อนเงื่อนไข
  if (prog0) await P.referralProgram.delete({ where: { systemId: SYS } }).catch(() => null);
  ...
  chk("M3.5-S1.2", "codeFor(สมาชิก 1) → { code = Customer.referralCode, link /ref/<code>, shareText แทน {link} ด้วย URL เต็ม (http) ไม่เหลือ {link} }", !!cf?.code && cf.code === c1?.referralCode && cf.link === `/ref/${cf.code}` && /https?:\/\/\S+\/ref\//.test(cf.shareText ?? "") && !/\{link\}/.test(cf.shareText ?? ""), "code ตรง", JSON.stringify(cf));
  ...
  // ORACLE-EDIT M3.5-S2.3: `phone: undefined` ทับเบอร์จนไม่มีทั้งเบอร์/อีเมล → createMember throw ทั้งชุด · ใส่อีเมลแทน (เบอร์ซ้ำทดสอบผ่าน device.phone)
  const Cc = await mkCust("เบอร์ซ้ำ", { phone: undefined, email: `dup-${tag}@qc-referral.test` });
  ...
  // ORACLE-EDIT M3.5-S3.2: แต้ม/การแปลงอยู่ใน consumer pos.sale.paid (ตั้งแต่ M2.8) → ระบายคิวก่อนอ่านผล
  { const { drainAll } = await import("@/lib/outbox-consumers"); await drainAll(); }
  ...
  chk("M3.5-S3.2", "บิล PAID ฿600 ≥ ขั้นต่ำ (consumer pos.sale.paid → evaluateConversion PURCHASE) → status REWARDED convertedAt conversionRef {saleId, netSatang} rewardedAt · ผู้แนะนำ +300 แต้ม (ledger EARN refType REFERRAL refId referralId) · เพื่อนได้ voucher FIXED 100 origin REFERRAL originRef {referralId} หมดอายุ +30 วัน · referrerRewardRef {ledgerId} refereeRewardRef {voucherId} · event referral.converted · MemberActivity REFERRAL_CONVERTED ของ A",
    rB2?.status === "REWARDED" && !!rB2.convertedAt && rB2.conversionRef?.saleId === (sB.saleId ?? sB.id) && !!rB2.rewardedAt && (await bal(A.customerId)) - balA0 === 300 && !!ledA && !!vB && vB.kind === "FIXED" && Number(vB.value) === 10_000 /* ORACLE-EDIT M3.5-S3.2: Voucher FIXED เก็บสตางค์ · ฿100 = 10000 */ && vB.originRef?.referralId === rB.id && Math.abs((new Date(vB.expiresAt).getTime() - Date.now()) / 86_400_000 - 30) < 1.5 && rB2.referrerRewardRef?.ledgerId === ledA?.id && rB2.refereeRewardRef?.voucherId === vB.id && !!obC && !!(await P.memberActivity.findFirst({ where: { customerId: A.customerId, type: "REFERRAL_CONVERTED", refId: rB.id } })),
  ...
  chk("M3.5-S5.2", "leaderboard 90 วัน: อันดับ 1 = A {referred ≥ 6 (ไม่นับ REJECTED), converted 6 (B,G,H1,H2,H3 + …), pointsEarned 300 (ORACLE-EDIT M3.5-S5.2: H1–H3 ติดเพดานตาม S4.1 ⇒ ไม่มี +10)} · stats {referredMembers ≥ 6, conversionPct 0–100, costPerMemberSatang ≥ 0, first90dSpendSatang ≥ 0, vsAvgPct number}", top?.customerId === A.customerId && top.converted >= 5 && top.referred >= top.converted && top.pointsEarned === 300 && (stt?.referredMembers ?? 0) >= 6 && stt.conversionPct >= 0 && stt.conversionPct <= 100 && stt.costPerMemberSatang >= 0 && stt.first90dSpendSatang >= 0 && typeof stt.vsAvgPct === "number", "A นำ", `top=${JSON.stringify(top)} stats=${JSON.stringify(stt)}`);
  const fm = await R.referralsForMember(ctx, owner, A.customerId);
  const m360 = await M.getMember360(ctx, owner, A.customerId);
  chk("M3.5-S5.3", "referralsForMember(A) → { code = codeA, link, referred, converted ≥ 5, pointsEarned 300, tree[{refereeId, name, status, convertedAt}] มี B (REWARDED) } · getMember360(A) มี referrals summary (counters.referrals หรือ referral block) · thana (ป่าตอง) เห็นรายการ (A home ป่าตอง) · unit scope ทำงาน", fm?.code === codeA && fm.converted >= 5 && fm.pointsEarned === 300 && Array.isArray(fm.tree) && fm.tree.some((t: Any) => t.refereeId === B.customerId && t.status === "REWARDED") && (m360?.counters?.referrals != null || m360?.referrals != null) && ((await R.listReferrals(ctx, thana, { referrerCustomerId: A.customerId, take: 5 }))?.items?.length ?? 0) > 0, "การ์ด 360", `fm=${JSON.stringify({ code: fm?.code === codeA, conv: fm?.converted, pts: fm?.pointsEarned, tree: fm?.tree?.length })} c360=${JSON.stringify(m360?.counters?.referrals ?? m360?.referrals)?.slice(0, 80)}`);
  ...
  // ═══ S7 UI / ภาพ ═══
  const pages = { page: read("src/app/app/sys/[id]/member/referrals/page.tsx"), liff: read("src/app/m/[slug]/referral/page.tsx"), r: read("src/app/ref/[code]/page.tsx") + read("src/app/r/[code]/route.ts") };
