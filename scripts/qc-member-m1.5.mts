...
try {
  ...
  chk("M1.5-S3.3", "nav.ts: members status ready · path /member/members · หน้าเดิม /member/customers → redirect ไปหน้าใหม่ (ไม่ทิ้ง 2 หน้ารายชื่อ) · /member/members/[id] มีจริง", navMembers?.status === "ready" && navMembers.path === "/member/members" && /redirect\(/.test(oldCustomers) && existsSync("src/app/app/sys/[id]/member/members/[id]/page.tsx") && existsSync("src/app/app/sys/[id]/member/members/page.tsx"), "ready + redirect", `nav=${JSON.stringify(navMembers)} redirect=${/redirect\(/.test(oldCustomers)}`);
  ...
  const page = read(PAGE); const page360 = read("src/app/app/sys/[id]/member/members/[id]/page.tsx");
