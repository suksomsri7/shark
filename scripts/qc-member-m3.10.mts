...
let tid = ""; let SYS = "";
let settingsSnap: { settings: unknown; program: Any } | null = null;
...
try {
  ...
  // ORACLE-EDIT M3.10-finally: ข้อสอบเปลี่ยนตั้งค่ารีวิว/แจ้งเตือน/แนะนำเพื่อน (S3.4–S3.6 · S3.8) แต่ไม่คืน → m3.4 S1.1 แดงตามมา · จำ AppSystem.settings + ReferralProgram ไว้คืนใน finally
  settingsSnap = { settings: (await prisma.appSystem.findUnique({ where: { id: SYS }, select: { settings: true } }))?.settings ?? null, program: await P.referralProgram?.findUnique?.({ where: { systemId: SYS } }).catch(() => null) ?? null };
  const BASE = process.env.QC_BASE ?? "http://127.0.0.1:3215";
  const http = async (method: string, path: string, key: string | null, body?: unknown, extra: Record<string, string> = {}) => { const r = await fetch(`${BASE}/api/v1${path}`, { method, headers: { ...(key ? { authorization: `Bearer ${key}` } : {}), "content-type": "application/json", ...(method === "GET" ? {} : { "idempotency-key": `m310-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` }), ...extra }, body: body ? JSON.stringify(body) : undefined }).catch(() => null); if (!r) return { status: 0, body: null, headers: new Headers() }; const j = await r.json().catch(async () => null); /* ORACLE-EDIT M3.10-S2/S3: แกน REST ตอบ { data, requestId } (respond.ts · ข้อสอบ M1.11/M2.10 อ่าน .body.data) → คลี่ data ออกให้ข้อเช็กอ่าน body.x · ข้อผิดพลาดคง { error } */ return { status: r.status, body: j && typeof j === "object" && "requestId" in j && "data" in j ? j.data : j, headers: r.headers }; };
  ...
  const toolNames = new Set(toolOps.map((o) => o.tool?.name ?? o.tool)); // ORACLE-EDIT M3.10-S2.1: op.tool เป็น { name, … } (แบบ M1.11)
  ...
  const csvBuf = rpCsv ? new Uint8Array(await rpCsv.arrayBuffer()) : new Uint8Array(); const csvText = (csvBuf[0] === 0xef && csvBuf[1] === 0xbb && csvBuf[2] === 0xbf ? "\ufeff" : "") + new TextDecoder().decode(csvBuf); // ORACLE-EDIT M3.10-S3.7: Response.text() ตัด BOM ทิ้งเอง (TextDecoder ปริยาย) → ตรวจไบต์ EF BB BF จริง
  // ORACLE-EDIT M3.10-S3.7: member.report.view อยู่ในชุด member-read/operate ตั้งแต่ M1.11 (scopes.ts:147) → ทดสอบ 403 ด้วยคีย์ operate ที่ตัด report.view ออก
  const kNoReport = await (async () => { const k = await ak.createApiKey({ tenantId: tid }, "QC M3.10 no-report", { scopes: scopes.expandBundles(["member-operate"]).filter((x: string) => x !== "member.report.view"), systemId: SYS }); keyIds.push(k.id); return k.rawKey as string; })();
  const rpNo = await http("GET", "/member/reports/overview", kNoReport);
  ...
  if (settingsSnap && SYS) {
    ...
    await d(() => prisma.appSystem.update({ where: { id: SYS }, data: { settings: (snap.settings ?? {}) as Any } }));
    if (snap.program) { const { id: _i, createdAt: _c, updatedAt: _u, ...rest } = snap.program; await d(() => P.referralProgram.update({ where: { systemId: SYS }, data: rest })); } else await d(() => P.referralProgram.deleteMany({ where: { systemId: SYS } }));
  }
