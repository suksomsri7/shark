...
try {
  ...
  const partyShared = cX?.partyId ? ((await P.accountContact.count({ where: { partyId: cX.partyId } })) + (await P.crmContact.count({ where: { partyId: cX.partyId } })) + (await P.chatContact.count({ where: { partyId: cX.partyId } }))) > 0 : false;
