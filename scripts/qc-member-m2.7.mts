...
//   applyOnSale(ctx, { saleId, customerId, unitId, cart, choices }, tx) — `cart` บังคับ (แก้ 10 ก.ย. หลัง M2.7 ชี้ว่าสัญญาเดิมไม่มียอดบิล · ห้ามพึ่ง cache ในหน่วยความจำ) → { tierDiscountSatang, voucherUseIds[], pointsBurned, pointsLedgerId|null, giftCardTxnId|null, giftCardSatang, lines[], totalDiscountSatang } — ทำใน tx ที่ POS ส่งมา: voucher.redeem(tx) ทุกใบ · point.burnFifo(tx) refType "PosSale" refId saleId idempotencyKey `pos-burn-${saleId}` · giftcard.use(tx) (ตรวจ PIN จริง) · ผิดพลาดข้อใด → throw (tx ทั้งก้อน rollback · ไม่เขียนบางส่วน) · คูปองไม่ทำที่นี่ (POS ทำเองอยู่แล้ว)
...
try {
  ...
  const ap = (await prisma.$transaction((tx) => M.applyOnSale(ctx, { saleId, customerId: X, unitId: E.units.patong, cart, choices: { voucherIds: [v1.id], points: 1000, giftCard: { number: gc.number, pin: gc.pin, satang: 100_000 } } }, tx))) as Any;
  ...
  const rb = await fails(() => prisma.$transaction(async (tx) => { await M.applyOnSale(ctx, { saleId: `${saleId}-rb`, customerId: X, unitId: E.units.patong, cart, choices: { voucherIds: [v2.id], points: 500, giftCard: { number: gc2.number, pin: gc2.pin, satang: 50_000 } } }, tx); throw new Error("ROLLBACK-QC"); }));
  ...
  const eUsed = await fails(() => prisma.$transaction((tx) => M.applyOnSale(ctx, { saleId: `${saleId}-x1`, customerId: X, unitId: E.units.patong, cart, choices: { voucherIds: [v1.id] } }, tx)));
  const ePin = await fails(() => prisma.$transaction((tx) => M.applyOnSale(ctx, { saleId: `${saleId}-x2`, customerId: X, unitId: E.units.patong, cart, choices: { voucherIds: [v2.id], giftCard: { number: gc2.number, pin: "000000", satang: 10_000 } } }, tx)));
  const ePts = await fails(() => prisma.$transaction((tx) => M.applyOnSale(ctx, { saleId: `${saleId}-x3`, customerId: X, unitId: E.units.patong, cart, choices: { points: 5000 } }, tx)));
