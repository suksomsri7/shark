...
try {
  ...
  const mkCust = async (nm: string, o: { line?: boolean; email?: boolean; phone?: boolean; consents?: string[] }) => {
    ...
    if (o.line) await P.memberChannelIdentity.create({ data: { tenantId: tid, customerId: c.customerId, channel: "LINE", externalId: `U-nt-${tag}-${Math.random().toString(36).slice(2, 8)}`, linkedBy: "MANUAL" /* ORACLE-EDIT M3.6-S2.2: linkedBy บังคับ — เดิม create ล้มเงียบใน .catch ไม่มีใครมี LINE */ } }).catch(() => null);
  ...
  // ORACLE-EDIT M3.6-S2.1: member-expected.json ไม่มี firstName → เทียบกับชื่อจริงใน DB
  const m0First = (await P.customer.findUnique({ where: { id: E.members[0].id }, select: { firstName: true } }))?.firstName;
  chk("M3.6-S2.1", "renderTemplate แทนทุกตัว (ซ้ำได้) · ตัวแปรไม่มีค่า → '' ไม่เหลือปีกกา · buildVars(สมาชิก 1) → ชื่อ = ชื่อจริง · ร้าน · ระดับ · แต้ม (ตัวเลข) · ลิงก์กระเป๋า URL เต็ม /m/<slug>/wallet · extra ผ่านมา", r1 === "สวัสดีคุณสมชาย คุณมี 1200 แต้ม (สมชาย)" && r2 === "หมดอายุ 31 ธ.ค. ลิงก์ " && vars?.ชื่อ === m0First && !!m0First && !!vars.ร้าน && !!vars.ระดับ && /^\d+$/.test(String(vars.แต้ม)) && new RegExp(`^https?://.+/m/${mq.MQC.tenantSlug}/wallet`).test(vars.ลิงก์กระเป๋า ?? "") && vars.voucher === "ส่วนลด ฿100", "render ถูก", `r1=${r1} r2=${r2} vars=${JSON.stringify(vars)}`);
  ...
  // ORACLE-EDIT M3.6-S4.1: เดิม AND `line.length < 1` กับ `=== 1` ในเช็คเดียวหลัง runDue(08:01) = ขัดกันเอง · อ่านค่า ณ แต่ละจุดเก็บไว้ (เข้มเท่าเดิม: หลังส่ง 0 · หลัง 07:30 ยัง 0 · หลัง 08:01 = 1)
  const lineAtSend = dQ.calls.line.length;
  const early = await N.runDue(ctx, { now: thaiAt(7, 30, 1), deps: dQ.deps });
  const lineAtEarly = dQ.calls.line.length;
  ...
  const lineBeforeDigest = dD.calls.line.length; // ORACLE-EDIT M3.6-S5.1: แบบเดียวกับ S4.1 (อ่าน ณ ก่อน runDue)
  ...
  const W = await mkCust("ไม่มีเบอร์", { line: true, phone: false, email: true, consents: ["LINE", "SMS"] }); // ORACLE-EDIT M3.6-S7.3: createMember ต้องมีเบอร์หรืออีเมล · SMS อ่านจากเบอร์เท่านั้น
  const dS3 = mkDeps();
  ...
  chk("M3.6-S8.3", "ภาพ 30: notifications-owner desktop+mobile 200 ไม่ล้น (ตาราง 8 แถว · ชิป 4 ช่อง · แถบสรุปเดือน) · notifications-edit-owner desktop 200 (คลิกแถว 'แต้มใกล้หมดอายุ' → แผงขวา แท็บ 4 · ตัวแปร · ตัวอย่างบับเบิล · 30/7 วันก่อน · สวิตช์ quiet/consent · ปุ่มทดสอบ/บันทึก) · thana (ไม่มีสิทธิ์) → 404 (กติกา 404-not-403 §6.4 เหมือนหน้าตั้งค่าอื่น) หรือ 403/redirect", ok("owner", "notifications-owner", "desktop") && ok("owner", "notifications-owner", "mobile") && ok("owner", "notifications-edit-owner", "desktop") && [404, 403, 302, 307].includes(r("thana", "notifications-thana", "desktop")?.status ?? (r("thana", "notifications-thana", "desktop")?.finalUrl?.includes("/member") && !r("thana", "notifications-thana", "desktop")?.finalUrl?.includes("notifications") ? 302 : 0)), "200 ×3 + 403", `${r("owner", "notifications-owner", "desktop")?.status}/${r("owner", "notifications-owner", "mobile")?.status}/${r("owner", "notifications-edit-owner", "desktop")?.status} thana=${r("thana", "notifications-thana", "desktop")?.status} missing=${JSON.stringify(r("owner", "notifications-edit-owner", "desktop")?.missing)}`);
