...
try {
  ...
  chk("M3.11-S5.1", "layout /m ฝังสะพาน push (m-push-bridge · ฟัง message push-token · เรียก /api/v1/member/me/push-devices) · REST ลงทะเบียน (session ของคนที่เพิ่งสมัคร) → 201 → MemberPushDevice {customerId, expoToken, platform ios} · ลงซ้ำ token เดิม → 200 ไม่ซ้ำแถว", /m-push-bridge/.test(layoutM) && /push-token/.test(layoutM) && /me\/push-devices/.test(layoutM) && [200, 201].includes(reg.status) /* ORACLE-EDIT M3.11-S5.1: แกน REST ตอบ 200 เสมอ (แบบ M3.10) */ && !!dev && (dev.token ?? dev.expoToken) /* ORACLE-EDIT M3.11-S5.1: คอลัมน์ MemberPushDevice.token (member.prisma:570) */ === tok && dev.platform === "ios" && [200, 201].includes(reg2) && (await P.memberPushDevice.count({ where: { customerId: c1?.customerId ?? "-" } })) === 1, "device ok", `bridge=${/m-push-bridge/.test(layoutM)} reg=${reg.status} dev=${!!dev} reg2=${reg2} n=${await P.memberPushDevice.count({ where: { customerId: c1?.customerId ?? "-" } }).catch(() => -1)}`);
  const pushCalls: Any[] = [];
