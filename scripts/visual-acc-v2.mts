...
const PAGES: Record<string, PageSpec[]> = {
  ...
  // ─────────── WO 8.3 — สิทธิ์ผู้ใช้งาน (§9.4 · g13) + การเชื่อมต่อ (§9.5 · g14/g14-menu) ───────────
  "8.3": [
    {
      name: "settings-permissions-matrix",
      path: `/app/sys/${SYS}/account/settings/permissions?s=matrix`,
      note: "ตั้งค่า › สิทธิ์ผู้ใช้งาน › สิทธิ์การใช้งาน — เทียบ g13 (ปุ่มบทบาท · ตาราง 8×7 · แถวเพดานอนุมัติ)",
      expect: [
        "ตั้งค่า",
        "สิทธิ์ผู้ใช้งาน",
        "ผู้ใช้งาน",
        "สิทธิ์การใช้งาน",
        "การเชื่อมต่อ",
        "กำหนดสิทธิ์แต่ละบทบาทต่อหมวดงาน",
        "เจ้าของ",
        "ผู้จัดการ",
        "พนักงานขาย",
        "ผู้อนุมัติ",
        ...
        "คลังเอกสาร",
        "เพดานอนุมัติ",
        "อนุมัติเอกสารได้ไม่เกิน",
        "ต่อรายการ (เว้นว่าง = ไม่จำกัด)",
        "บันทึก",
        "ยกเลิก",
      ],
      onlyDevice: "desktop",
    ...
    {
      name: "settings-permissions-matrix-approver",
      path: `/app/sys/${SYS}/account/settings/permissions?s=matrix&r=role2`,
      note: "บทบาท 'ผู้อนุมัติ' — ช่องเพดานอนุมัติมีค่า 50,000.00 จริง (เหมือน g13)",
      // ค่า 50,000.00 อยู่ใน `value` ของช่องกรอก ไม่ใช่ข้อความบนหน้า → ตรวจที่บล็อก assert (matrix-cap)
      expect: ["ผู้อนุมัติ", "เพดานอนุมัติ", "อนุมัติเอกสารได้ไม่เกิน"],
      onlyDevice: "desktop",
    },
    {
      name: "settings-permissions-users",
      path: `/app/sys/${SYS}/account/settings/permissions?s=users`,
      note: "ตั้งค่า › สิทธิ์ผู้ใช้งาน › ผู้ใช้งาน — ตารางคนที่มีสิทธิ์บัญชี + ปุ่มเชิญผู้ใช้",
      expect: [
        "ผู้ใช้งาน",
        "เชิญผู้ใช้",
        "ชื่อ",
    ...
    },
    {
      ...
      path: `/app/sys/${SYS}/account/settings/connections?s=shark`,
      note: "ตั้งค่า › การเชื่อมต่อ › ระบบใน SHARK — เทียบ g14 (การ์ด 2 คอลัมน์ · ชิปสถานะ · toggle · บัญชีที่ใช้ · แถบกติกา)",
      expect: [
        "การเชื่อมต่อ · ระบบใน SHARK",
        "สมุดบัญชีเล่มนี้รับข้อมูลจากระบบต่อไปนี้อัตโนมัติ",
        "ระบบใน SHARK",
        "e-Tax Invoice",
        "เร็ว ๆ นี้",
        "แอปภายนอก/API",
        ...
        "เปิดระบบก่อน",
        "กติกาความปลอดภัย:",
        "ระบบที่ยังไม่เชื่อมจะไม่ลงบัญชีให้ (ไม่เชื่อม = ไม่ลงบัญชีให้)",
      ],
      onlyDevice: "desktop",
    },
    {
      name: "settings-connections-menu",
      path: `/app/sys/${SYS}/account/settings/connections?s=shark`,
      note: "เมนู ⋯ ของการ์ด POS — เทียบ g14-menu (ดูรายการที่ลง · ตัดการเชื่อม)",
      click: ['[data-testid="conn-menu-POS"] button'],
      waitAfterClick: 400,
      expect: ["ดูรายการที่ลง", "ตัดการเชื่อม"],
      onlyDevice: "desktop",
    },
    {
      name: "settings-connections-api",
      path: `/app/sys/${SYS}/account/settings/connections?s=api`,
      note: "ตั้งค่า › การเชื่อมต่อ › แอปภายนอก/API — คีย์ API · webhook (เหตุการณ์บัญชี) · Zapier/Make · bank feed 🕓",
      expect: [
        "คีย์ API",
        "ชื่อคีย์",
        "ตัวขึ้นต้น",
        "สร้างคีย์",
        "Webhook (แจ้งเหตุการณ์บัญชีไปยังระบบอื่น)",
        "ปลายทาง",
        "เหตุการณ์",
        "เมื่ออนุมัติเอกสารบัญชี",
        "เมื่อบันทึกรับ/จ่ายเงิน",
        "เมื่อใบแจ้งหนี้ถูกชำระครบ",
        "เมื่อปิดงวดบัญชี",
        "ยิงทดสอบ",
        "การส่งล่าสุด",
        "เชื่อมกับแอปอื่น",
    ...
    {
      name: "settings-permissions-mobile",
      path: `/app/sys/${SYS}/account/settings/permissions?s=users`,
      note: "มือถือ 390: ตารางผู้ใช้งานกลายเป็นการ์ด (§13) — ต้องไม่ล้นแนวนอน",
      expect: ["ผู้ใช้งาน", "ปนัดดา ขายดี", "เพดานอนุมัติ", "หัวข้อตั้งค่า"],
      onlyDevice: "mobile",
    },
    {
      name: "settings-connections-mobile",
      path: `/app/sys/${SYS}/account/settings/connections?s=shark`,
      note: "มือถือ 390: การ์ดการเชื่อมต่อเรียง 1 คอลัมน์ — ต้องไม่ล้นแนวนอน",
      expect: ["ระบบใน SHARK", "หน้าร้าน (POS)", "เชื่อมแล้ว", "กติกาความปลอดภัย:"],
      onlyDevice: "mobile",
    },
  ],
...
const ASSERT_MAP: Record<string, Record<string, Record<string, number | string>>> = {
  ...
  // WO 8.3 — ค่าบนหน้าสิทธิ์/การเชื่อมต่อ ต้องเป็นค่าที่ seed ตั้งไว้จริง
  "8.3": {
    "settings-permissions-matrix-approver": {
      // เพดานอนุมัติของบทบาท "ผู้อนุมัติ" (5,000,000 สตางค์ = ฿50,000.00 · เกณฑ์ผ่าน BLUEPRINT 8.3)
      "matrix-cap": ((E.permissions?.approverCapSatang ?? 0) / 100).toLocaleString("th-TH", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
    },
    "settings-permissions-users": {
      [`user-cap-${E.permissions?.memberships?.["approver@siamdive-qc.test"] ?? ""}`]: `฿${(
        (E.permissions?.approverCapSatang ?? 0) / 100
      ).toLocaleString("th-TH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      [`user-cap-${E.permissions?.memberships?.["sales@siamdive-qc.test"] ?? ""}`]: "ไม่จำกัด",
    },
    "settings-connections": {
      // ชิปสถานะของการ์ด POS/HR — พิสูจน์ว่าหน้าอ่าน AccountSystemLink จริง ไม่ใช่ข้อความตายตัว
      "conn-toggle-POS-autoPost": "on",
      "conn-toggle-MEMBER-autoPost": "off",
    },
  },
